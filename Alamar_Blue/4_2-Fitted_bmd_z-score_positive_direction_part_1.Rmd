---
title: "4.2_Fitted_z_score_positive_direction"
author: "Jory Curry"
...
filter_2 <- create_dataset(z_dat, n_samples = params$sample_size) %>%
  group_by(chemical) %>%
  filter(chemical %in% NaNs[[1]]) %>%
  mutate(direction = NA,
         fitted_resp = NA,
...
confident_fitted_hits <- confident_hits(summary_dat = fitted_hits, reject_hit_conf_under = 0)
...
saveRDS(fitted_output_summary, file = paste0("Data/", "fitted_z_score_", dir,"_output_summary_", params$sample_size, "_samples.RDS"))
saveRDS(bmds_fitted, file = paste0("Data/", "fitted_z_score_", dir,"_bmds_", params$sample_size, "_samples.RDS"))
```
