...
bmr_thresh_spearman <- readRDS(file = paste0("Data/", "spearman_rcurvepBMR.rds"))
```
...
rm(bmd_dat_nobin_neg_spearman, input_spearman_tib)
gc()
```
...
#spearman Negative Direction RCurveP - Entire code chunk is for 'Not binned' data
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%#
...
#confident_curves_spearman <- confident_hits(summary_dat = simulated_curves_spearman_summary, reject_hit_conf_under = 0.5)
# If Error says "out of bounds", reduce conf filter

#Data Wrangling for nice labels
simulated_curves_spearman_summary <- simulated_curves_spearman_summary %>%
  group_by(chemical) %>%
...
curve_hits_spearman <- curve_hits_spearman %>%
  group_by(chemical) %>%
  mutate(x_label_num = if_else(condition = conc == lowest_conc, true = 0, false = (10^conc)*lowest_dose_treatment_group)) %>%
  mutate(x_label = factor(x_label_num, levels = c(0, lowest_dose_treatment_group, lowest_dose_treatment_group*10, lowest_dose_treatment_group*100, lowest_dose_treatment_group*1000, lowest_dose_treatment_group*10000, lowest_dose_treatment_group*100000, lowest_dose_treatment_group*1000000, lowest_dose_treatment_group*10000000, lowest_dose_treatment_group*100000000, lowest_dose_treatment_group*1000000000, lowest_dose_treatment_group*10000000000), ordered = TRUE))
...
p_spearman_simulated_no_processing <- ggplot(NULL, aes(x_label, resp)) +
  geom_point(
  ...
  xlab("Dose (mg/L)") +
  ylab(paste0("Normalized ", params$endpoint, "'s", " rho similarity score"))
...
chemicalNames <- unique(simulated_curves_spearman_summary$chemical)
p_list <- list()
for (k in 1:length(chemicalNames)) {
  labels <- simulated_curves_spearman_summary %>%
    filter(chemical == chemicalNames[k]) %>%
    select(x_label_num) %>%
    pull() %>%
    unique() %>%
    print.numeric()
  temp_simulated <- simulated_curves_spearman_summary %>%
    filter(chemical == chemicalNames[k])
  temp_curve_hits <- curve_hits_spearman %>%
    filter(chemical == chemicalNames[k])
  temp_bmds <- bmds_spearman %>%
    filter(chemical == chemicalNames[k])
...
print(p_spearman_simulated_no_processing)
```

```{r alternative_save_the_resutls, include=FALSE}
yleft <-
  grid::textGrob(paste0("Normalized ", params$endpoint, "'s", " rho similarity score"),
           rot = 90,
...
ggsave(
  plot = p_spearman_simulated_no_processing,
  filename = paste0(
...
#print(p_rcurvep_spearman_distribution_bmds)
```

```{r save_plots_again, include=FALSE}
# ggsave(
#   plot = p_rcurvep_spearman_distribution_bmds,
#   filename = paste0(
