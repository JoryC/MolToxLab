...
for (j in 1:length(chemicalNames)) {
  for (i in 1:6) {
    ...
    new_row <- matrix(data = c(0,0,0,1), nrow = 1, ncol = 4, )
...
lfishBehavDat <- fishBehavDat %>%
  ...
  group_by(plate_id, dose, embryo_id) %>%
  summarise(value = mean(value, na.rm = TRUE)) %>%
  mutate(time_end = 1, is_VC = if_else(condition = dose == 0, true = TRUE, false = FALSE)) %>%
  as.data.frame() %>%
  ungroup() %>%
  select(plate_id, embryo_id, is_VC, time_end, value)
...
```{r simi_endpoints, include = FALSE}
simi_endps <- list(seq(1, 1, by = 1)) #One per minute
names(simi_endps) <- params$metric
simi_endpoints <- list()
for (i in chemicalNames) {
  temp <-
