---
title: "EDA Zebrafish Embryo Behaviour Assay"
author: "Jory Curry"
...
<!-- #   group_by(Cycle, Dose) %>% -->
<!-- #   mutate(sd = sd(totaldist, na.rm = TRUE)) %>% -->
<!-- #   mutate(totaldist = mean(totaldist, na.rm = TRUE)) %>% -->
<!-- #   summarise(totaldist, sd) %>% -->
<!-- #   ggplot(aes(x = Cycle, y = totaldist, fill = Dose, color = Dose)) + -->
<!-- #   geom_col(position = "dodge") + -->
<!-- #   geom_errorbar(aes(ymin = totaldist, ymax = totaldist+sd), width = .5, position = position_dodge(.9)) + -->
...
fishBehavDatBin5 %>%
  ...
  filter(Cycle != "Acclimation") %>%
  group_by(Cycle, Dose) %>%
  summarise(se = sd(totaldist, na.rm = TRUE)/sqrt(length(totaldist)), totaldist = mean(totaldist, na.rm = TRUE)) %>%
  ggplot(aes(x = Cycle, y = totaldist, fill = Dose, color = Dose)) +
  geom_col(position = "dodge") +
  geom_errorbar(aes(ymin = totaldist, ymax = totaldist+se), width = .5, position = position_dodge(.9)) +
  scale_fill_viridis_d() +
  ylab("Mean Total Distance") +
  xlab("")
  
```
```{r Dunnett's_test, include=FALSE}
...
names(temp_dun)[1] <- cycle_filter[i]
dun_list[i] <- temp_dun
```

```{r summary_all_chems , echo=FALSE}
summary_fishBehavDatBin <- fishBehavDatBin5 %>%
  ...
  summarise(se = sd(.$params$value, na.rm = TRUE)/sqrt(length(.$params$value)), value = mean(.$params$value, na.rm = TRUE))
```
...
<!-- # ```{r sumtotaldistance_binnedDat, echo=FALSE} -->
<!-- # fishBehavDatBin5 %>% -->
<!-- #   filter(plate_id == "DMSO") %>% -->
<!-- #   mutate(Cycle = factor( -->
<!-- #     Cycle, -->
...
<!-- #   filter(Cycle != "Acclimation") %>% -->
<!-- #   group_by(Cycle, Dose) %>% -->
<!-- #   summarise(sd = sd(totaldist, na.rm = TRUE), totaldist = sum(totaldist, na.rm = TRUE)) %>% -->
<!-- #   ggplot(aes(x = Cycle, y = totaldist, fill = Dose, color = Dose)) + -->
<!-- #   geom_col(position = "dodge") + -->
<!-- #   geom_errorbar(aes(ymin = totaldist-sd, ymax = totaldist+sd), position = position_dodge(.9), width = .5) + -->
<!-- #   scale_fill_viridis_d() + -->
<!-- #   ylab("Sum of Total Distance") + -->
<!-- #   xlab("") -->
<!-- #    -->
<!-- # ``` -->
...
simi_norm_tib %>%
  ...
  ylim(-100, 100)
```
After running an ANCOVA, it can be seen that 4TPP comes back as significant. However, Dunnett's test does not return any results, so it can't be determined from the Dunnett's test which dose group is statistically different from the controls.\
\
...
\
Stay updated! Check out my [github repo](https://github.com/JoryC/MolToxLab).