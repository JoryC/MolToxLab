...
ss_summary %>%
  mutate(lowest_conc = 10^(lowest_conc), highest_conc = 10^(highest_conc), POD_cil = 10^(POD_cil), POD_med = 10^(POD_med), POD_ciu = 10^(POD_ciu), RNGE = RNGE/1000000) %>%
  select(chemical, Bin, RNGE, TRSH, lowest_conc, highest_conc, POD_cil, POD_med, POD_ciu, hit_confidence) %>%
  kable(col.names = c("Chemical", "Bin",  "Direction", "BMR Threshold", "Low Dose", "High Dose", "BMDL", "BMD", "BMDU", "Hit Confidence"), caption = "Table xx. Summary of all the benchmark doses (BMDs) of every chemical, binning method, and direction") %>%
  kable_styling() %>%
