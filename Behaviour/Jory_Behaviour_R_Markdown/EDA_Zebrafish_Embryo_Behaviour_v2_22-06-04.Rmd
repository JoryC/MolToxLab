...
Figure 6 showed that the `emptydur` variable can be used reliably to filter out observations with poor data quality. The 'empty duration' variable ranges from 0s to 60s and indicates how long the observation was not able to detect a fish in the well. An arbitrary cutoff value of `r 60*0.25` seconds will be used to determine if an observation was of poor-quality and therefore, should be converted to `NA`. By doing this, the confidence in the accuracy of observations can be increased across the entire data set.
...
```{r pre_process_1, include=FALSE, class.source ='fold-hide', message=FALSE, warning=FALSE}
...
```{r binning_fishBehavDat, include=FALSE}
...
```{r summary_all_chems, include=FALSE}
...
```{r dunnett_summary_all_chems, include=FALSE}
...
```{r meantotaldist_BinnedDat, echo=FALSE}
...
```{r summary_all_chems_no_bin, include=FALSE}
...
```{r joined_dunnet_summary_no_bin, include=FALSE}
...
```{r simi_endpoints, include = FALSE}
...
```{r normalized_data, include=FALSE}
...
for(i in chemicalNames) {
  ...
  simi_norm[[i]][, "Group"] = factor(rep(c("A", "B", "C"), times = (nrow(simi_norm[[i]]) /
                                                                      9), each = 3))
...
for (i in chemicalNames) {
  dunnett_list[[i]][,"pval"] <- p.adjust(dunnett_list[[i]][,"pval"], method = "fdr")
...
```{r normalized_data_to_tibble, include=FALSE}
doseData <- doseData %>%
  mutate(dose = Dose) #Renaming dose variable to inner_join
simi_norm_tib <-
  as_tibble(rbindlist(simi_norm)) #Convert from list to tibble
...
```{r normalized_data_ggplot, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center', fig.cap="Figure 16. Dose response relationship of normalized pearson's similarity scores from 26 different chemicals -- `totaldsit` variable", fig.width=10, fig.height=7.5, out.height="100%", out.width="125%"}
simi_norm_tib %>%
  mutate(dose = factor(
    dose,
    levels = c("Control", "Dose5", "Dose4", "Dose3", "Dose2", "Dose1"),
  ...
  ylim(-100, 100)
```
After running an ANCOVA, it can be seen that 4TPP and Malathion come back as significant. However, Dunnett's test does not return any results, so it can't be determined from the Dunnett's test which dose group is statistically different from the controls.\
\
However, it could still be possible to calculate a benchmark dose for each chemical.

### Benchmark Dose

```{r prep_4_rcurvep, include=FALSE}
...
```{r estimating_BMR, echo=FALSE, warning=FALSE, message=FALSE}
set.seed(42069)
...
```{r estimating_BMR_2, echo=FALSE, fig.cap= "Figure 17. Summary of Benchmark response estimation", fig.align='center', warning=FALSE, message=FALSE}
bmr_output <-
  estimate_dataset_bmr(simi_norm_tib_4rcurvep_act, plot = TRUE)
```

```{r estimating_BMR_3, echo=FALSE}
bmr_output$outcome %>%
  ...
  kable(
    ...
    align = 'llrrrr',
    caption = "Table 7. Summary of the estimated BMR"
  ) %>%
  kable_styling() %>%
  scroll_box()
```
...
```{r lowest_BMR, include=FALSE}
bmr_vect <- c(bmr_output$outcome$bmr_exp, bmr_output$outcome$bmr_ori)
bmr_thresh <- sort(bmr_vect) %>%
  min()
```
...
```{r calculating_benchmark_dose, include = FALSE}
bmd_dat <- combi_run_rcurvep(
  simi_norm_tib_4rcurvep, 
  n_samples = 10, #Increase this number to 100 or 1000 for better results (takes a long time to run)
  keep_sets = "act_set", 
  TRSH = bmr_thresh,
...
```{r calculating_benchmark_dose_2, echo=FALSE, collapse=TRUE}
sum_bmd_dat <- summarize_rcurvep_output(bmd_dat)
sum_bmd_dat
```

```{r calculating_benchmark_dose_3, echo=FALSE}
sum_bmd_dat$act_summary %>%
  ...
  kable(col.names = c("Chemical", "Low Dose", "High Dose", "BMD - low confidence", "BMD", "BMD - high confidence", "Hit Confidence"), caption = "Table 8. Summary of all the benchmark doses (BMDs) of every chemical") %>%
  kable_styling() %>%
  row_spec(which(sum_bmd_dat$act_summary$POD_med < sum_bmd_dat$act_summary$highest_conc), bold = TRUE, background = "gray") %>%
  scroll_box()
```
...
\
It is notable that certain chemicals had a BMD successfully calculated (highlighted in gray), including 4TPP which came back as statistically signficant from the ANCOVA, but not Malathion.


## Analysis - Similarity - Light bin

```{r pre_processing_4_pipeline_L, include=FALSE}
...
for (i in chemicalNames){
ancova_list[[i]]$`Pr(>F)` <- p.adjust(ancova_list[[i]]$`Pr(>F)`, method = "fdr")
} # p value adjustment
...
Note to self: find out why some embryos are being flagged as outliers/poor quality data but only in the dark cycle...
...
simi_norm_tib_4rcurvep_act <- combi_run_rcurvep(
  ...
  keep_sets = "act_set",
  TRSH = seq(5, 95, by = 5),
  RNGE = 1000000 # Changing direction for dark cycle since most fish are becoming more active during the dark and with increasing chemical exposure doses...
)
```
