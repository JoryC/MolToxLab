---
title: "Report for ANCOVA and Dunnett's of spearman SS"
author: "Jory Curry"
...
cor_dat <- fishBehavDat %>%
  ...
  dplyr::summarize(cor = cor(x = as.matrix(.), method = "spearman", use = "pairwise.complete.obs"))
...
p1 <- fishBehavDat %>%
  ...
  geom_text(label = paste("spearman's r = ", as.character(cor_dat$Control_1)), mapping = aes(x = 10, y = 100), inherit.aes = FALSE)
...
p2 <- fishBehavDat %>%
  ...
  geom_text(label = paste("spearman's r = ", as.character(cor_dat$Control_9)), mapping = aes(x = 10, y = 100), inherit.aes = FALSE)
...
```{r 47simi_endpoints_spearman, include = FALSE}
simi_endps_spearman <- list(seq(1, 30, by = 1)) #Segments - One per minute
...
```{r 48normalized_data_spearman, include=FALSE}
simi_norm_nobin_spearman <- lapply(simi_endpoints_nobin_spearman, simi_normalize)
```

```{r 49summary_statistics_spearman, include=FALSE}
for(i in chemicalNames) {
...
temp_spearman <-
  plyr::ldply(simi_norm_nobin_spearman) %>% #Convert from list to tibble
  mutate(.id = NULL)
...
anova_group_eff_test <- temp_spearman %>%
  group_by(plate_id, dose) %>%
  nest() %>%
...
```{r 50ANCOVA_spearman, include=FALSE}
anova_list_nobin_spearman <- sapply(simi_norm_nobin_spearman, combinedanova)

anova_comb_nobin_spearman <- as_tibble(plyr::ldply(anova_list_nobin_spearman))
anova_comb_nobin_spearman <- anova_comb_nobin_spearman %>%
  na.omit() %>%
...
anova_comb_nobin_spearman %>%
  select(.id, Df, `F value`, `Pr(>F)`, endpoint) %>%
  kable(
    col.names = c("Chemical", "Degrees of Freedom", "F-Value", "P-value", "Endpoint"),
    align = 'llrrr',
    caption = "Table 6. Summary table of ANOVA results from the `spearman` endpoint after pooling sub-groups"
  ) %>%
  kable_styling()
```
...
```{r 51Dunnett_spearman, include=FALSE}
set.seed(024)
...
dunnett_comb_nobin_spearman$dose = substr(dunnett_comb_nobin_spearman$dose,
                           start = 1,
                           stop = nchar(dunnett_comb_nobin_spearman$dose) - 8) #Here we are fixing the dose column... the dose column has the test dose related to the control... but we just want to see what the test dose is without it giving us redundant information about the comparison to the control for every observation...
...
```{r 53normalized_data_to_tibble_spearman, include=FALSE}
doseData <- doseData %>%
...
```{r 54normalized_data_ggplot, include=FALSE, message=FALSE, warning=FALSE, fig.align='center', fig.cap="Figure 16. Dose response relationship of normalized spearman's similarity scores from 26 different chemicals -- `totaldsit` variable", fig.width=10, fig.height=7.5, out.height="100%", out.width="125%"}
Dunnett_results <- dunnett_comb_nobin_spearman %>%
  rename(plate_id = .id) %>%
  mutate(dose = str_split(string = dose, pattern = "-", simplify = TRUE)[,1])

ANOVA_results <- anova_comb_nobin_spearman %>%
  rename(plate_id = .id) %>%
  filter(Df == 5) %>%
...
p_both_endpoints <- list(p_spearman)
names(p_both_endpoints) <- c("spearman")
```
...
ggsave(
  plot = p_both_endpoints$spearman,
  filename = "Output/ANCOVA_Dunnetts_spearman.png",
  device = "png",
  scale = 3,
  units = "px",
  width = 1920,
...
rm(
  ...
  outliers,
  spearman,
  summarystats_list_nobin_spearman,
  summarystats_list_nobin_spearman,
  simi_endpoints_nobin_spearman,
  simi_endpoints_nobin_spearman,
  cor_dat_long,
  p1,
  p2,
  ...
  filter_out,
  dunnett_temp,
  dunnett_list_nobin_spearman,
  dunnett_comb_nobin_spearman,
  dunnett_list_nobin_spearman,
  BoxPlotOutliers,
  BoxPlotOutliersRemove,
  ancova_list_nobin_spearman,
  ancova_list_nobin_spearman,
  fishBehavDat,
  fishBehavDatBin5,
  fishBehavDatBinLD,
...
saveRDS(object = simi_norm_tib_4rcurvep_spearman, file = paste0("Data/", "simi_norm_tib_4rcurvep_spearman.rds"))
```