...
simulated_curves_pearson <- sum_bmd_dat_nobin_neg_pearson$result$resp_set
simulated_curve_bmds_pearson <- sum_bmd_dat_nobin_neg_pearson$act_summary %>%
  group_by(chemical) %>%
  summarise(POD_med = POD_med, POD_cil = POD_cil, hit_confidence = hit_confidence, lowest_conc = lowest_conc)

simulated_curves_spearman <- sum_bmd_dat_nobin_neg_spearman$result$resp_set
simulated_curve_bmds_spearman <- sum_bmd_dat_nobin_neg_spearman$act_summary %>%
  group_by(chemical) %>%
  summarise(POD_med = POD_med, POD_cil = POD_cil, hit_confidence = hit_confidence, lowest_conc = lowest_conc)

p_pearson <- simulated_curves_pearson %>%
  group_by(chemical, sample_id, conc) %>%
  ggplot(aes(x = conc, y = resp, group = sample_id), color = "gray75") +
  geom_line(alpha = 0.3) +
  geom_hline(yintercept = -bmr_thresh_pearson, linetype = "dashed", color = "red") +
  geom_vline(data = simulated_curve_bmds_pearson, aes(xintercept = POD_med), color = "blue") +
  geom_vline(data = simulated_curve_bmds_pearson, aes(xintercept = POD_cil), color = "green") +
  geom_text(data = simulated_curve_bmds_pearson, mapping = aes(label = paste("Hit =", hit_confidence), x = lowest_conc+0.75), y = -85, inherit.aes = FALSE, size = 3) +
  facet_wrap(~chemical, scales = "free") +
  ylim(-100,50) +
  theme_minimal() +
  theme(panel.border = element_rect(colour = "grey50", fill = NA))

p_spearman <- simulated_curves_spearman %>%
  group_by(chemical, sample_id, conc) %>%
  ggplot(aes(x = conc, y = resp, group = sample_id), color = "gray75") +
  geom_line(alpha = 0.3) +
  geom_hline(yintercept = -bmr_thresh_spearman, linetype = "dashed", color = "red") +
  geom_vline(data = simulated_curve_bmds_spearman, aes(xintercept = POD_med), color = "blue") +
  geom_vline(data = simulated_curve_bmds_spearman, aes(xintercept = POD_cil), color = "green") +
  geom_text(data = simulated_curve_bmds_spearman, mapping = aes(label = paste("Hit =", hit_confidence), x = lowest_conc+0.75), y = -85, inherit.aes = FALSE, size = 3) +
  facet_wrap(~chemical, scales = "free") +
  ylim(-100,50) +
  theme_minimal() +
  theme(panel.border = element_rect(colour = "grey50", fill = NA))
...
names(p_list) <- c("Pearson", "Spearman")
```

##### Simulated Curve Plots with BMDs (Blue), and BMDLs (Green) {.tabset}
```{r 61.2Simulated_curves, echo=FALSE, results='asis'}
for (i in seq_along(p_list)){
  temp <- p_list[[i]]
  cat("######", names(p_list)[i], " \n")
  print(temp)
...
## Analysis - Similarity Scores - Removed Chemicals

### No bins

```{r xxpre_processing_4_pipeline, include=FALSE}
lfishBehavDat <- fishBehavDat %>%
  # filter(plate_id %in% c("BPAF", "24DMP")) %>%
  mutate(value = .[[params$value]], dose = Dose_mg_L) %>%
  filter(time_end > 20) %>% # For the analysis, we don't want the 20 minute acclimation period
  filter(!(plate_id %in% c("17beta_estradiol", "24DMP", "4TOP", "BPAF", "Fenitrothion"))) %>%
  mutate(time_end = time_end-20) %>% # Display time 1-30 minutes
  select(plate_id, embryo_id, is_VC, time_end, value) %>%
  na.omit() # Pipeline is picky about the data frame
lfishBehavDat <- split(as.data.frame(lfishBehavDat), ~ plate_id) # Split into lists
...
```{r xxVisual_of_how_to_calculate_simi_enpoints, echo = FALSE}
p1 <- fishBehavDat %>%
  filter(plate_id == "BPA", time_end > 20, Dose == "Dose2") %>%
  mutate(time_end = time_end-20) %>%
  ggplot(aes(x = time_end, y = totaldist, color = embryo_id)) +
...
```{r xx.xVisual_of_how_to_calculate_simi_enpoints, echo = FALSE}
 cor_dat <- fishBehavDat %>%
  filter(plate_id == "BPA", time_end > 20, embryo_id %in% c("Dose2_1", "Control_1", "Control_2", "Control_3", "Control_4", "Control_5", "Control_6", "Control_7", "Control_8", "Control_9")) %>%
  select(embryo_id, time_end, totaldist) %>%
  arrange(embryo_id) %>%
...
```{r xx.xxVisual_of_how_to_calculate_simi_enpoints, echo = FALSE}
cor_dat_long <- cor_dat %>%
    select(-Dose2_1) %>%
    pivot_longer(cols = 1:9,names_to = "embryo_id", values_to = "totaldist")
mean_cor_dat <- mean(cor_dat_long$totaldist)
...
```{r xxsimi_endpoints_pearson, include = FALSE}
simi_endps_pearson <- list(seq(1, 30, by = 1)) #One per minute
names(simi_endps_pearson) <- "pearson"
simi_endpoints_nobin_pearson <- list()
for (i in chemicalNames[-c(1, 4, 8, 12, 19)]) {
  temp <-
    create_simi_endpoints(lfishBehavDat[[i]],
                          segments = simi_endps_pearson,
                          metric = "pearson")
...
```{r xx.xsimi_endpoints_spearman, include = FALSE}
simi_endps_spearman <- list(seq(1, 30, by = 1)) #One per minute
names(simi_endps_spearman) <- "spearman"
simi_endpoints_nobin_spearman <- list()
for (i in chemicalNames[-c(1, 4, 8, 12, 19)]) {
...
```{r xxnormalized_data_pearson, include=FALSE}
simi_norm_nobin_pearson <- lapply(simi_endpoints_nobin_pearson, simi_normalize)
```

```{r xx.xnormalized_data_spearman, include=FALSE}
simi_norm_nobin_spearman <- lapply(simi_endpoints_nobin_spearman, simi_normalize)
```

```{r xxsummary_statistics_pearson, include=FALSE}
for(i in chemicalNames[-c(1, 4, 8, 12, 19)]) {
  simi_norm_nobin_pearson[[i]]$dose <- as.factor(simi_norm_nobin_pearson[[i]]$dose)
  simi_norm_nobin_pearson[[i]][, "Group"] = factor(rep(c("A", "B", "C"), times = (nrow(simi_norm_nobin_pearson[[i]]) /
                                                                      9), each = 3))
...
```{r xx.xsummary_statistics_spearman, include=FALSE}
for(i in chemicalNames[-c(1, 4, 8, 12, 19)]) {
  simi_norm_nobin_spearman[[i]]$dose <- as.factor(simi_norm_nobin_spearman[[i]]$dose)
  simi_norm_nobin_spearman[[i]][, "Group"] = factor(rep(c("A", "B", "C"), times = (nrow(simi_norm_nobin_spearman[[i]]) /
                                                                      9), each = 3))
...
```{r xxANCOVA_pearson, include=FALSE}
ancova_list_nobin_pearson <- sapply(simi_norm_nobin_pearson, combinedancova)

for (i in 1:length(chemicalNames[-c(1, 4, 8, 12, 19)])){
  ancova_list_nobin_pearson[[i]]$`Pr(>F)` <- p.adjust(p = ancova_list_nobin_pearson[[i]]$`Pr(>F)`, method = "fdr")
}
...
```{r xx.xANCOVA_spearman, include=FALSE}
ancova_list_nobin_spearman <- sapply(simi_norm_nobin_spearman, combinedancova)
...
```{r xx.xANCOVA_Results_Summary, echo = FALSE}
ancova_summary_both_endpoints <- rbind(ancova_comb_nobin_pearson, ancova_comb_nobin_spearman)
...
```{r xxDunnett_pearson, include=FALSE}
dunnett_list_nobin_pearson <- sapply(simi_norm_nobin_pearson, combineddunnett) %>%
  setNames(., chemicalNames[-c(1, 4, 8, 12, 19)]) %>%
  as.array()
#Dunnett's Test requires a bunch of wrangling
dunnett_temp <-
  list() #Take Dunnett's test results without any of the fancy summary information and shove it into a named list
...
```{r xx.xDunnett_spearman, include=FALSE}
dunnett_list_nobin_spearman <- sapply(simi_norm_nobin_spearman, combineddunnett) %>%
  setNames(., chemicalNames[-c(1, 4, 8, 12, 19)]) %>%
  as.array()
#Dunnett's Test requires a bunch of wrangling
...
```{r xx.xDunnett_results_summary, echo=FALSE}
# Any significant results?
Nobin_dunnet_results_both_endpoints <- rbind(dunnett_comb_nobin_pearson, dunnett_comb_nobin_spearman)
...
```{r xxnormalized_data_to_tibble_pearson, include=FALSE}
doseData <- doseData %>%
  mutate(dose = Dose) #Renaming dose variable to inner_join
simi_norm_tib_pearson <-
  as_tibble(rbindlist(simi_norm_nobin_pearson)) #Convert from list to tibble
...
```{r xx.xnormalized_data_to_tibble_spearman, include=FALSE}
doseData <- doseData %>%
  mutate(dose = Dose) #Renaming dose variable to inner_join
simi_norm_tib_spearman <-
  as_tibble(rbindlist(simi_norm_nobin_spearman)) #Convert from list to tibble
...
```{r xxnormalized_data_ggplot, include=FALSE, message=FALSE, warning=FALSE, fig.align='center', fig.cap="Figure 16. Dose response relationship of normalized pearson's similarity scores from 26 different chemicals -- `totaldsit` variable", fig.width=10, fig.height=7.5, out.height="100%", out.width="125%"}
p_pearson <- simi_norm_tib_pearson %>%
  mutate(dose = factor(
    dose,
    levels = c("Control", "Dose5", "Dose4", "Dose3", "Dose2", "Dose1"),
...
##### Plots by endpoint {.tabset}
```{r xx.xmeantotaldist_BinnedDat, echo = FALSE, results = 'asis', warning=FALSE, message=FALSE}
for (i in seq_along(p_both_endpoints)){
  temp <- p_both_endpoints[[i]]
  cat("######", names(p_both_endpoints)[i], " \n")
  print(temp)
...
```{r xxprep_4_rcurvep, include=FALSE}
simi_norm_tib_4rcurvep_spearman <- simi_norm_tib_spearman %>%
  rename(resp = endpoint_value_norm,
         chemical = plate_id) %>%
  filter(is_VC == 0) %>%
...
```{r xx.xprep_4_rcurvep, include=FALSE}
simi_norm_tib_4rcurvep_pearson <- simi_norm_tib_pearson %>%
  rename(resp = endpoint_value_norm,
         chemical = plate_id) %>%
  filter(is_VC == 0) %>%
...
```{r xxestimating_BMR, include=FALSE, warning=FALSE, message=FALSE}
set.seed(42069)
...
```{r xx.xestimating_BMR, include=FALSE, warning=FALSE, message=FALSE}
set.seed(42069)
...
```{r xxestimating_BMR_2, include=FALSE, fig.cap= "Figure 17. Summary of Benchmark response estimation", fig.align='center', warning=FALSE, message=FALSE}
bmr_output_pearson <-
  estimate_dataset_bmr(simi_norm_tib_4rcurvep_act_pearson, plot = TRUE)
```

```{r xx.xestimating_BMR_2, include=FALSE, fig.cap= "Figure 17. Summary of Benchmark response estimation", fig.align='center', warning=FALSE, message=FALSE}
bmr_output_spearman <-
  estimate_dataset_bmr(simi_norm_tib_4rcurvep_act_spearman, plot = TRUE)
```

```{r xx.xxestimating_BMR_2, include=FALSE, fig.cap= "Figure 17. Summary of Benchmark response estimation", fig.align='center', warning=FALSE, message=FALSE}
bmr_output_summary <- list(plot(bmr_output_pearson), plot(bmr_output_spearman))
names(bmr_output_summary) <- c("Pearson", "Spearman")
bmr_output <- rbind(bmr_output_pearson$outcome, bmr_output_spearman$outcome)
```
...
##### Benchmark Response Plots {.tabset}
```{r xx.xxxestimating_BMR_2, echo=FALSE, fig.cap= "Figure 17. Summary of Benchmark response estimation", fig.align='center', warning=FALSE, message=FALSE, results='asis'}
for (i in seq_along(bmr_output_summary)){
  temp <- bmr_output_summary[[i]]
  cat("######", names(bmr_output_summary)[i], " \n")
  print(temp)
...
```{r xxestimating_BMR_3, echo=FALSE}
bmr_output %>%
  select(endpoint, qc, cor_exp_fit, cor_lm_fit, bmr_exp, bmr_ori) %>%
  kable(
    col.names = c(
...
```{r xx.xlowest_BMR, include=FALSE}
bmr_output_spearman$outcome[which(is.na(bmr_output_spearman$outcome))] = 0
bmr_thresh_spearman <- bmr_output_spearman$outcome %>%
    mutate(bmr = if_else(condition = .$cor_lm_fit > .$cor_exp_fit, true = .$bmr_ori, false = .$bmr_exp)) %>%
    pull(bmr)
...
```{r xxcalculating_benchmark_dose, include = FALSE}
bmd_dat_nobin_neg_pearson <- combi_run_rcurvep(
  simi_norm_tib_4rcurvep_pearson, 
  n_samples = params$sample_size, #Increase this number to 100 or 1000 for better results (takes a long time to run)
  keep_sets = c("act_set", "resp_set"), 
...
```{r xx.xcalculating_benchmark_dose, include = FALSE}
bmd_dat_nobin_neg_spearman <- combi_run_rcurvep(
  simi_norm_tib_4rcurvep_spearman, 
  n_samples = params$sample_size, #Increase this number to 100 or 1000 for better results (takes a long time to run)
  keep_sets = c("act_set", "resp_set"), 
...
```{r xxcalculating_benchmark_dose_2, echo=FALSE, collapse=TRUE}
sum_bmd_dat_nobin_neg_pearson <- summarize_rcurvep_output(bmd_dat_nobin_neg_pearson)
sum_bmd_dat_nobin_neg_spearman <- summarize_rcurvep_output(bmd_dat_nobin_neg_spearman)
...
```{r xx.xsimulated_curveps, include = FALSE}
simulated_curves_pearson <- sum_bmd_dat_nobin_neg_pearson$result$resp_set
simulated_curve_bmds_pearson <- sum_bmd_dat_nobin_neg_pearson$act_summary %>%
  group_by(chemical) %>%
  summarise(POD_med = POD_med, POD_cil = POD_cil, hit_confidence = hit_confidence, lowest_conc = lowest_conc)
...
##### Simulated Curve Plots with BMDs (Blue), and BMDLs (Green) {.tabset}
```{r xx.xxSimulated_curves, echo=FALSE, results='asis'}
for (i in seq_along(p_list)){
  temp <- p_list[[i]]
  cat("######", names(p_list)[i], " \n")
  print(temp)
...
```{r xxcalculating_benchmark_dose_3, echo=FALSE}
sum_bmd_dat_nobin_neg %>%
  mutate(lowest_conc = 10^(lowest_conc), highest_conc = 10^(highest_conc), POD_cil = 10^(POD_cil), POD_med = 10^(POD_med), POD_ciu = 10^(POD_ciu)) %>%
  select(chemical, endpoint, lowest_conc, highest_conc, POD_cil, POD_med, POD_ciu, hit_confidence) %>%
  kable(col.names = c("Chemical", "endpoint", "Low Dose", "High Dose", "BMD - low confidence", "BMD", "BMD - high confidence", "Hit Confidence"), caption = "Table 9. Summary of all the benchmark doses (BMDs) of every chemical") %>%
...
## Summary Plots (include Dunnett's results in plots)
