...
Since we do not know what the benchmark response is for each one of the chemicals, a method will be used that can estimate a benchmark response based off of the given data. The RcurveP package provides an automated process to tselect an appropriate BMR based off of the data. This method uses bootstrapping to 100s to 1000s of curves that will help determine what a good estimate is for the benchmark response. This process can be a bit time-intensive to run.
...
```{r 96.1calculating_benchmark_dose_3, include=FALSE}
...
## Similarity Score Analysis Summary

```{r summary_table_ss_1, echo = FALSE}
sum_bmd_dat_D_neg$act_summary <- sum_bmd_dat_D_neg$act_summary %>%
  mutate(Bin = "Dark")
sum_bmd_dat_D_pos$act_summary <- sum_bmd_dat_D_pos$act_summary %>%
  mutate(Bin = "Dark")

sum_bmd_dat_L_neg$act_summary <- sum_bmd_dat_L_neg$act_summary %>%
  mutate(Bin = "Light")
sum_bmd_dat_L_pos$act_summary <- sum_bmd_dat_L_pos$act_summary %>%
  mutate(Bin = "Light")

sum_bmd_dat_nobin_neg$act_summary <- sum_bmd_dat_nobin_neg$act_summary %>%
  mutate(Bin = "None")
sum_bmd_dat_nobin_pos$act_summary <- sum_bmd_dat_nobin_pos$act_summary %>%
  mutate(Bin = "None")

ss_summary <- rbind(sum_bmd_dat_D_neg$act_summary, sum_bmd_dat_D_pos$act_summary, sum_bmd_dat_L_neg$act_summary, sum_bmd_dat_L_pos$act_summary, sum_bmd_dat_nobin_neg$act_summary, sum_bmd_dat_nobin_pos$act_summary) %>%
  arrange(chemical, desc(hit_confidence))
```

```{r Summary_Table, echo = FALSE}
ss_summary %>%
  mutate(lowest_conc = round(10^(lowest_conc), digits = 8), highest_conc = round(10^(highest_conc), digits = 3), POD_cil = round(10^(POD_cil), digits = 6), POD_med = round(10^(POD_med), digits = 6), POD_ciu = round(10^(POD_ciu), digits = 3), RNGE = RNGE/1000000) %>%
  select(chemical, Bin, RNGE, lowest_conc, highest_conc, POD_cil, POD_med, POD_ciu, hit_confidence) %>%
  kable(col.names = c("Chemical", "Bin",  "Direction", "Low Dose", "High Dose", "BMDL", "BMD", "BMDU", "Hit Confidence"), caption = "Table xx. Summary of all the benchmark doses (BMDs) of every chemical, binning method, and direction") %>%
  kable_styling() %>%
      row_spec(which((ss_summary$hit_confidence >= 0.5) & (ss_summary$POD_med < ss_summary$highest_conc)), bold = TRUE, background = "gray") %>%
  scroll_box()
```

<!-- # Delete this later -- Rearranging sections to better make summary figures and tables -->
...
ancova_sum <- rbind(ancova_comb_nobin, ancova_comb_L, ancova_comb_D)

ancova_sum %>%
  filter(is.significant == TRUE, Df == 5) %>%
  arrange(.id) %>%
  select(.id, Bin, Df, `F value`, `Pr(>F)`) %>%
  kable(
    col.names = c("Chemical", "Bin","Degrees of Freedom", "F-Value", "P-value"),
    align = 'lllrr',
    caption = "Table xx. Summary table of significant ANCOVA results (p < 0.05) from the `totaldist` variable after controling for within-group variability ofr all binning methods"
  ) %>%
  kable_styling() %>%
  scroll_box()
```
