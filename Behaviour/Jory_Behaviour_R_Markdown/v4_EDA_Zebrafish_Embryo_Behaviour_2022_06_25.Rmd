...
bmds_pearson <- simulated_curves_pearson_summary %>%
  group_by(chemical) %>%
  summarise(median_POD = unique(median_POD), POD_cil = unique(POD_cil), mean_POD = unique(mean_POD), POD_med = unique(POD_med), ciu_POD = unique(ciu_POD), cil_POD = unique(cil_POD))

confident_curves_pearson <- curve_hits_pearson %>%
...
temp_name <- NULL
for (i in 1:length(confident_curves_pearson)) {
temp <- unique(confident_curves_pearson[[i]]$conf_POD)
temp_name[i] <- temp
}
names(confident_curves_pearson) <- c(temp_name)
