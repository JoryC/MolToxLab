...
\
\
Chemicals with no observable dose-response effects that could be removed from the analysis
...
From the many plots above, it can be seen that there are a few chemicals that appear to have a flat dose-response relationship for all metrics investigated:

- 17-beta estradiol
- 24DMP
- 4TOP (potential effects in light only)
- BPAF
- Fenitrothion (perhaps non-monotonic potential effect)

Therefore, they could be removed before conducting a benchmark dose analysis
...
<!-- # ```{r 52Dunnett_results_pearson, include=FALSE} -->
<!-- # # Any significant results? -->
<!-- # dunnett_comb_nobin_pearson %>% -->
<!-- #   filter(is.significant == TRUE) %>% -->
...
<!-- # ```{r 52.1Dunnett_results_spearman, include=FALSE} -->
<!-- # # Any significant results? -->
<!-- # dunnett_comb_nobin_spearman %>% -->
<!-- #   filter(is.significant == TRUE) %>% -->
<!-- #   select(.id, dose, diff, pval) %>% -->
...
```{r 56estimating_BMR, include=FALSE, warning=FALSE, message=FALSE}
set.seed(42069)
...
```{r 56.1estimating_BMR, include=FALSE, warning=FALSE, message=FALSE}
set.seed(42069)
...
<!-- # ```{r 58.1estimating_BMR_3, include=FALSE} -->
<!-- # bmr_output_spearman$outcome %>% -->
<!-- #   select(endpoint, qc, cor_exp_fit, cor_lm_fit, bmr_exp, bmr_ori) %>% -->
<!-- #   kable( -->
<!-- #     col.names = c( -->
<!-- #       "Endpoint", -->
...
<!-- # ``` -->
<!-- #  -->
<!-- # ```{r 58.2BMR_Summary, echo=FALSE} -->
<!-- # bmr_output_summary <- rbind(bmr_output_pearson$outcome, bmr_output_spearman$outcome) -->
<!-- #  -->
<!-- # bmr_output_summary %>%   -->
<!-- #   select(endpoint, qc, cor_exp_fit, cor_lm_fit, bmr_exp, bmr_ori) %>% -->
<!-- #   kable( -->
...
```{r 61.0Data_Wrangling_Function, include = FALSE}

summarise_rcurvep_results <- function(resp_set, act_set, act_summary, reject_hit_conf_under = 0.5) {
  ...
  mutate(median_POD = if_else(condition = hit_confidence < reject_hit_conf_under, true = unique(highest_conc), false = median_POD)) %>%
  ...
  mutate(mean_POD = if_else(condition = is.na(mean(mean_POD, na.rm = TRUE)), true = unique(highest_conc), false = mean(mean_POD, na.rm = TRUE))) %>%
    mutate(mean_POD = if_else(condition = hit_confidence < reject_hit_conf_under, true = unique(highest_conc), false = mean_POD)) %>%
    mutate(cil_POD = if_else(condition = POD == lowest_conc, true = unique(highest_conc), false = POD)) %>%
    mutate(cil_POD = if_else(condition = is.na(quantile(cil_POD, prob = c(0.05), na.rm = TRUE)), true = unique(highest_conc), false = quantile(cil_POD, prob = c(0.05), na.rm = TRUE))) %>%
    mutate(cil_POD = if_else(condition = hit_confidence < reject_hit_conf_under, true = unique(highest_conc), false = cil_POD)) %>%
    mutate(ciu_POD = if_else(condition = POD == lowest_conc, true = unique(highest_conc), false = POD)) %>%
    mutate(ciu_POD = if_else(condition = is.na(quantile(ciu_POD, prob = c(0.95), na.rm = TRUE)), true = unique(highest_conc), false = quantile(ciu_POD, prob = c(0.95), na.rm = TRUE))) %>%
    mutate(ciu_POD = if_else(condition = hit_confidence < reject_hit_conf_under, true = unique(highest_conc), false = ciu_POD))
...
bmd_results <- function(summary_dat){
  summary_dat %>%
    group_by(chemical) %>%
    summarise(median_POD = unique(median_POD), mean_POD = unique(mean_POD), ciu_POD = unique(ciu_POD), cil_POD = unique(cil_POD))
}


confident_hits <- function(summary_dat, reject_hit_conf_under = 0.5) {
  confident_hit_result <- summary_dat %>%
    filter(hit == 1, hit_confidence >= 0.5) %>%
  group_by(chemical, sample_id) %>%
  mutate(POD_diff_cil = abs(POD-cil_POD), POD_diff_ciu = abs(POD-ciu_POD), POD_diff_median = abs(POD-median_POD), POD_diff_mean = abs(POD-mean_POD), smallest_diff = min(c(POD_diff_cil, POD_diff_ciu, POD_diff_mean, POD_diff_median)), conf_POD = if_else(condition = smallest_diff == POD_diff_cil, true = "cil", false = if_else(condition = smallest_diff == POD_diff_ciu, true = "ciu", false = if_else(condition = smallest_diff == POD_diff_median, true = "median", false = "mean")))) %>%
  ungroup() %>%
  group_by(conf_POD) %>%
...
temp <- unique(confident_curves_pearson[[i]]$conf_POD)
temp_name[i] <- temp
}
...
bmds_pearson <- bmd_summary(simulated_curves_pearson_summary)

curve_hits_pearson <- simulated_curves_pearson_summary %>%
  filter(hit == 1)

confident_curves_pearson <- confident_hits(simulated_curves_pearson_summary)
...
p_pearson <- ggplot(NULL, aes(conc, resp)) +
  ...
  geom_text(data = simulated_curve_bmds_pearson, mapping = aes(label = paste("Hit =", hit_confidence), x = lowest_conc+1), y = -85, inherit.aes = FALSE, size = 3) +
...
simulated_curves_spearman <- sum_bmd_dat_nobin_neg_spearman$result$resp_set # To merge with other sets

simulated_curve_bmds_spearman <- sum_bmd_dat_nobin_neg_spearman$act_summary %>%
  group_by(chemical) %>%
  summarise(POD_med = POD_med, POD_cil = POD_cil, hit_confidence = hit_confidence, lowest_conc = lowest_conc) # To merge with other sets

simulated_curves_spearman_act_set <- sum_bmd_dat_nobin_neg_spearman$result$act_set

simulated_curves_spearman_summary <- simulated_curves_spearman %>%
  inner_join(simulated_curves_spearman_act_set) %>%
  group_by(chemical, conc, sample_id) %>%
  summarise(endpoint = endpoint, resp = resp, corrected_resp = corrected_resp, correction = correction, POD = POD, highest_conc = highest_conc, hit = hit, lowest_conc = lowest_conc) %>%
  inner_join(simulated_curve_bmds_spearman) %>%
  mutate(resp = if_else(correction == 1, true = corrected_resp, false = resp)) %>%
  select(-corrected_resp) %>%
  mutate(median_POD = if_else(condition = POD == lowest_conc, true = unique(highest_conc), false = POD)) %>%
  mutate(median_POD = if_else(condition = is.na(median(median_POD, na.rm = TRUE)), true = unique(highest_conc), false = median(median_POD, na.rm = TRUE))) %>%
...
confident_curves_spearman <- curve_hits_spearman %>%
...
bmds_spearman <- simulated_curves_spearman_summary %>%
  group_by(chemical) %>%
...
p_spearman <- ggplot(NULL, aes(conc, resp)) +
  ...
  geom_line(data = confident_curves_spearman[[1]], mapping = aes(x = conc, y = resp, group = sample_id), color = "green") +
  geom_line(data = confident_curves_spearman[[2]], mapping = aes(x = conc, y = resp, group = sample_id), color = "yellow") +
  geom_line(data = confident_curves_spearman[[3]], mapping = aes(x = conc, y = resp, group = sample_id), color = "blue") +
  ...
  geom_vline(data = bmds_spearman, aes(xintercept = POD_med), color = "yellow") +
  geom_vline(data = bmds_spearman, aes(xintercept = mean_POD), color = "green") +
  # geom_vline(data = bmds_spearman, aes(xintercept = POD_cil), color = "purple") +
  geom_text(data = simulated_curve_bmds_spearman, mapping = aes(label = paste("Hit =", hit_confidence), x = lowest_conc+1), y = -85, inherit.aes = FALSE, size = 3) +
  facet_wrap(~chemical, scales = "free") +
  ylim(-100,50) +
  theme_minimal() +
  theme(panel.border = element_rect(colour = "grey50", fill = NA), panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
pearson_fitted <- simulated_curves_pearson_summary %>%
  select(endpoint, chemical, conc, resp) %>%
  group_by(chemical) %>%
  run_fit(
  .,
...
# Use the noise-filtered data from the curveP output
spearman_fitted <- simulated_curves_spearman_summary %>%
  select(endpoint, chemical, conc, resp) %>%
  group_by(chemical) %>%
  run_fit(
  .,
...
pearson_fitted_output_1 <- pearson_fitted_output$result$resp_set %>%
  inner_join(pearson_fitted_output$result$act_set) %>%
  group_by(chemical, conc, sample_id) %>%
  summarise(fitted_resp = fitted_resp, POD = POD, lowest_conc = lowest_conc, highest_conc = highest_conc, endpoint = endpoint, hit = hit) %>%
  inner_join(pearson_fitted_output$act_summary) %>%
...
pearson_fitted_output_summary <- pearson_fitted_output_1 %>%
  group_by(chemical, conc, sample_id) %>%
...
confident_fitted_hits_pearson <- fitted_hits_pearson %>%
  filter(hit_confidence >= 0.5) %>%
  group_by(chemical, sample_id) %>%
  mutate(POD_diff_med = abs(POD-POD_med), POD_diff_median = abs(POD-median_POD), POD_diff_mean = abs(POD-mean_POD), smallest_diff = min(c(POD_diff_med, POD_diff_mean, POD_diff_median)), conf_POD = if_else(condition = smallest_diff == POD_diff_med, true = "med", false = if_else(condition = smallest_diff == POD_diff_median, true = "median", false = "mean"))) %>%
  ungroup() %>%
...
spearman_fitted_output_1 <- spearman_fitted_output$result$resp_set %>%
  inner_join(spearman_fitted_output$result$act_set) %>%
  group_by(chemical, conc, sample_id) %>%
  summarise(fitted_resp = fitted_resp, POD = POD, lowest_conc = lowest_conc, highest_conc = highest_conc, endpoint = endpoint, hit = hit) %>%
  inner_join(spearman_fitted_output$act_summary) %>%
  select(endpoint, chemical, sample_id, conc, fitted_resp, lowest_conc, highest_conc, POD, POD_med, POD_cil, POD_ciu, hit, hit_confidence)

spearman_fitted_output_summary <- spearman_fitted_output_1 %>%
  group_by(chemical, conc, sample_id) %>%
  summarise(fitted_resp = unique(fitted_resp), lowest_conc = unique(lowest_conc), highest_conc = unique(highest_conc), POD = unique(POD), POD_cil = unique(POD_cil), POD_med = unique(POD_med), POD_ciu = unique(POD_ciu), hit = unique(hit), hit_confidence = unique(hit_confidence)) %>%
    mutate(median_POD = if_else(condition = POD == lowest_conc, true = unique(highest_conc), false = POD)) %>%
  mutate(median_POD = if_else(condition = is.na(median(median_POD, na.rm = TRUE)), true = unique(highest_conc), false = median(median_POD, na.rm = TRUE))) %>%
...
confident_fitted_hits_spearman <- fitted_hits_spearman %>%
  filter(hit_confidence >= 0.5) %>%
  group_by(chemical, sample_id) %>%
  mutate(POD_diff_med = abs(POD-POD_med), POD_diff_median = abs(POD-median_POD), POD_diff_mean = abs(POD-mean_POD), smallest_diff = min(c(POD_diff_med, POD_diff_mean, POD_diff_median)), conf_POD = if_else(condition = smallest_diff == POD_diff_med, true = "med", false = if_else(condition = smallest_diff == POD_diff_median, true = "median", false = "mean"))) %>%
  ungroup() %>%
...
p_pearson_fitted <- ggplot(NULL, aes(conc, resp)) +
  ...
  geom_line(data = confident_fitted_hits_pearson[[1]], mapping = aes(x = conc, y = fitted_resp, group = sample_id), color = "green") +
  geom_line(data = confident_fitted_hits_pearson[[2]], mapping = aes(x = conc, y = fitted_resp, group = sample_id), color = "yellow") +
  geom_line(data = confident_fitted_hits_pearson[[3]], mapping = aes(x = conc, y = fitted_resp, group = sample_id), color = "blue") +
  geom_hline(yintercept = -bmr_thresh_pearson, linetype = "dashed", color = "red") +
  geom_vline(data = bmds_fitted_pearson, aes(xintercept = median_POD), color = "blue") +
  geom_vline(data = bmds_fitted_pearson, aes(xintercept = POD_med), color = "yellow") +
  geom_vline(data = bmds_fitted_pearson, aes(xintercept = mean_POD), color = "green") +
  # geom_vline(data = bmds_fitted_pearson, aes(xintercept = POD_cil), color = "purple") +
  geom_text(data = pearson_fitted_output_summary, mapping = aes(label = paste("Hit =", hit_confidence), x = lowest_conc+1), y = -60, inherit.aes = FALSE, size = 3) +
...
p_spearman_fitted <- ggplot(NULL, aes(conc, resp)) +
  ...
  geom_line(data = confident_fitted_hits_spearman[[1]], mapping = aes(x = conc, y = fitted_resp, group = sample_id), color = "green") +
  geom_line(data = confident_fitted_hits_spearman[[2]], mapping = aes(x = conc, y = fitted_resp, group = sample_id), color = "yellow") +
  # geom_line(data = confident_fitted_hits_spearman[[3]], mapping = aes(x = conc, y = fitted_resp, group = sample_id), color = "blue") +
  geom_hline(yintercept = -bmr_thresh_spearman, linetype = "dashed", color = "red") +
  geom_vline(data = bmds_fitted_spearman, aes(xintercept = median_POD), color = "blue") +
  geom_vline(data = bmds_fitted_spearman, aes(xintercept = POD_med), color = "yellow") +
  geom_vline(data = bmds_fitted_spearman, aes(xintercept = mean_POD), color = "green") +
  # geom_vline(data = bmds_fitted_spearman, aes(xintercept = POD_cil), color = "purple") +
  geom_text(data = spearman_fitted_output_summary, mapping = aes(label = paste("Hit =", hit_confidence), x = lowest_conc+1), y = -60, inherit.aes = FALSE, size = 3) +
  facet_wrap(~chemical, scales = "free_x") +
  ylim(-100,50) +
  theme_minimal() +
  theme(panel.border = element_rect(colour = "grey50", fill = NA))
...
```{r 87.2Distribution_of_PODs, include = FALSE}
curve_hits_pearson %>%
  group_by(chemical) %>%
ggplot(data = ., aes(x = POD)) +
  ...
  geom_vline(aes(xintercept = cil_POD), color = "green") +
  geom_vline(aes(xintercept = ciu_POD), color = "purple") +
  xlim(-7, 3)
```
...
```{r 62calculating_benchmark_dose_3, include=FALSE}
sum_bmd_dat_nobin_neg %>%
  ...
  select(chemical, endpoint, lowest_conc, highest_conc, POD_cil, POD_med, POD_ciu, hit_confidence) %>%
  kable(col.names = c("Chemical", "endpoint", "Low Dose", "High Dose", "BMD - low confidence", "BMD", "BMD - high confidence", "Hit Confidence"), caption = "Table 9. Summary of all the benchmark doses (BMDs) of every chemical") %>%
  kable_styling() %>%
  row_spec(which(sum_bmd_dat_nobin_neg$POD_med < sum_bmd_dat_nobin_neg$highest_conc), bold = TRUE, background = "gray") %>%
  scroll_box()
```
...
The the create_auc_endpoints function is going to caluculate the area under the curve for each chemical
...
auc_all <- auc_all %>%
  ...
  mutate(Dose = str_split(string = embryo_id, pattern = "_", simplify = TRUE)[,1],
         embryo_id = str_split(string = embryo_id, pattern = "_", simplify = TRUE)[,2])
auc_join <- auc_all %>%
  filter(Dose == "Control") %>%
  group_by(plate_id, endpoint) %>%
  mutate(median_value = median(endpoint_value_log10), Dose = NULL, endpoint_value = NULL, endpoint_value_log10 = NULL, log_dose = NULL, Dose_mg_L = NULL)
auc_norm <- auc_all %>%
  full_join(auc_join, by = c(".id", "plate_id", "embryo_id", "endpoint")) %>%
  mutate(endpoint_value_norm = (endpoint_value_log10 - median_value)*100) %>%
  mutate(Cycle = if_else(condition = endpoint %in% c("Dark_1", "Dark_2", "Dark_3"), true = "Dark", false = "Light"))
```
...
```{r 95Rcurvep_obj_wrangling, include = FALSE}
## Light_1
simulated_curves_Light_1 <- Light_1_out$result$resp_set # To merge with other sets
simulated_curve_bmds_Light_1 <- Light_1_out$act_summary %>%
  group_by(chemical) %>%
  summarise(POD_med = POD_med, POD_cil = POD_cil, hit_confidence = hit_confidence, lowest_conc = lowest_conc) # To merge with other sets
simulated_curves_Light_1_act_set <- Light_1_out$result$act_set

simulated_curves_Light_1_summary <- simulated_curves_Light_1 %>%
  inner_join(simulated_curves_Light_1_act_set) %>%
  group_by(chemical, conc, sample_id) %>%
  summarise(endpoint = endpoint, resp = resp, corrected_resp = corrected_resp, correction = correction, POD = POD, highest_conc = highest_conc, hit = hit, lowest_conc = lowest_conc) %>%
  inner_join(simulated_curve_bmds_Light_1) %>%
  mutate(resp = if_else(correction == 1, true = corrected_resp, false = resp)) %>%
  select(-corrected_resp) %>%
  mutate(median_POD = if_else(condition = POD == lowest_conc, true = unique(highest_conc), false = POD)) %>%
  mutate(median_POD = if_else(condition = is.na(median(median_POD, na.rm = TRUE)), true = unique(highest_conc), false = median(median_POD, na.rm = TRUE))) %>%
...
confident_curves_Light_1 <- curve_hits_Light_1%>%
  filter(hit_confidence >= 0.5) %>%
  group_by(chemical, sample_id) %>%
  mutate(POD_diff_med = abs(POD-POD_med), POD_diff_median = abs(POD-median_POD), POD_diff_mean = abs(POD-mean_POD), smallest_diff = min(c(POD_diff_med, POD_diff_mean, POD_diff_median)), conf_POD = if_else(condition = smallest_diff == POD_diff_med, true = "med", false = if_else(condition = smallest_diff == POD_diff_median, true = "median", false = "mean"))) %>%
  ungroup() %>%
...
## Light_2
simulated_curves_Light_2 <- Light_2_out$result$resp_set # To merge with other sets
simulated_curve_bmds_Light_2 <- Light_2_out$act_summary %>%
  group_by(chemical) %>%
  summarise(POD_med = POD_med, POD_cil = POD_cil, hit_confidence = hit_confidence, lowest_conc = lowest_conc) # To merge with other sets
simulated_curves_Light_2_act_set <- Light_2_out$result$act_set

simulated_curves_Light_2_summary <- simulated_curves_Light_2 %>%
  inner_join(simulated_curves_Light_2_act_set) %>%
  group_by(chemical, conc, sample_id) %>%
  summarise(endpoint = endpoint, resp = resp, corrected_resp = corrected_resp, correction = correction, POD = POD, highest_conc = highest_conc, hit = hit, lowest_conc = lowest_conc) %>%
  inner_join(simulated_curve_bmds_Light_2) %>%
  mutate(resp = if_else(correction == 1, true = corrected_resp, false = resp)) %>%
  select(-corrected_resp) %>%
  mutate(median_POD = if_else(condition = POD == lowest_conc, true = unique(highest_conc), false = POD)) %>%
  mutate(median_POD = if_else(condition = is.na(median(median_POD, na.rm = TRUE)), true = unique(highest_conc), false = median(median_POD, na.rm = TRUE))) %>%
...
confident_curves_Light_2 <- curve_hits_Light_2%>%
  filter(hit_confidence >= 0.5) %>%
  group_by(chemical, sample_id) %>%
  mutate(POD_diff_med = abs(POD-POD_med), POD_diff_median = abs(POD-median_POD), POD_diff_mean = abs(POD-mean_POD), smallest_diff = min(c(POD_diff_med, POD_diff_mean, POD_diff_median)), conf_POD = if_else(condition = smallest_diff == POD_diff_med, true = "med", false = if_else(condition = smallest_diff == POD_diff_median, true = "median", false = "mean"))) %>%
  ungroup() %>%
...
## Light_3
simulated_curves_Light_3 <- Light_3_out$result$resp_set # To merge with other sets
simulated_curve_bmds_Light_3 <- Light_3_out$act_summary %>%
  group_by(chemical) %>%
  summarise(POD_med = POD_med, POD_cil = POD_cil, hit_confidence = hit_confidence, lowest_conc = lowest_conc) # To merge with other sets
simulated_curves_Light_3_act_set <- Light_3_out$result$act_set

simulated_curves_Light_3_summary <- simulated_curves_Light_3 %>%
  inner_join(simulated_curves_Light_3_act_set) %>%
  group_by(chemical, conc, sample_id) %>%
  summarise(endpoint = endpoint, resp = resp, corrected_resp = corrected_resp, correction = correction, POD = POD, highest_conc = highest_conc, hit = hit, lowest_conc = lowest_conc) %>%
  inner_join(simulated_curve_bmds_Light_3) %>%
  mutate(resp = if_else(correction == 1, true = corrected_resp, false = resp)) %>%
  select(-corrected_resp) %>%
  mutate(median_POD = if_else(condition = POD == lowest_conc, true = unique(highest_conc), false = POD)) %>%
  mutate(median_POD = if_else(condition = is.na(median(median_POD, na.rm = TRUE)), true = unique(highest_conc), false = median(median_POD, na.rm = TRUE))) %>%
...
confident_curves_Light_3 <- curve_hits_Light_3%>%
  filter(hit_confidence >= 0.5) %>%
  group_by(chemical, sample_id) %>%
  mutate(POD_diff_med = abs(POD-POD_med), POD_diff_median = abs(POD-median_POD), POD_diff_mean = abs(POD-mean_POD), smallest_diff = min(c(POD_diff_med, POD_diff_mean, POD_diff_median)), conf_POD = if_else(condition = smallest_diff == POD_diff_med, true = "med", false = if_else(condition = smallest_diff == POD_diff_median, true = "median", false = "mean"))) %>%
  ungroup() %>%
...
## Dark_1
simulated_curves_Dark_1 <- Dark_1_out$result$resp_set # To merge with other sets
simulated_curve_bmds_Dark_1 <- Dark_1_out$act_summary %>%
  group_by(chemical) %>%
  summarise(POD_med = POD_med, POD_cil = POD_cil, hit_confidence = hit_confidence, lowest_conc = lowest_conc) # To merge with other sets
simulated_curves_Dark_1_act_set <- Dark_1_out$result$act_set

simulated_curves_Dark_1_summary <- simulated_curves_Dark_1 %>%
  inner_join(simulated_curves_Dark_1_act_set) %>%
  group_by(chemical, conc, sample_id) %>%
  summarise(endpoint = endpoint, resp = resp, corrected_resp = corrected_resp, correction = correction, POD = POD, highest_conc = highest_conc, hit = hit, lowest_conc = lowest_conc) %>%
  inner_join(simulated_curve_bmds_Dark_1) %>%
  mutate(resp = if_else(correction == 1, true = corrected_resp, false = resp)) %>%
  select(-corrected_resp) %>%
  mutate(median_POD = if_else(condition = POD == lowest_conc, true = unique(highest_conc), false = POD)) %>%
  mutate(median_POD = if_else(condition = is.na(median(median_POD, na.rm = TRUE)), true = unique(highest_conc), false = median(median_POD, na.rm = TRUE))) %>%
...
confident_curves_Dark_1 <- curve_hits_Dark_1%>%
  filter(hit_confidence >= 0.5) %>%
  group_by(chemical, sample_id) %>%
  mutate(POD_diff_med = abs(POD-POD_med), POD_diff_median = abs(POD-median_POD), POD_diff_mean = abs(POD-mean_POD), smallest_diff = min(c(POD_diff_med, POD_diff_mean, POD_diff_median)), conf_POD = if_else(condition = smallest_diff == POD_diff_med, true = "med", false = if_else(condition = smallest_diff == POD_diff_median, true = "median", false = "mean"))) %>%
  ungroup() %>%
...
## Dark_2
simulated_curves_Dark_2 <- Dark_2_out$result$resp_set # To merge with other sets
simulated_curve_bmds_Dark_2 <- Dark_2_out$act_summary %>%
  group_by(chemical) %>%
  summarise(POD_med = POD_med, POD_cil = POD_cil, hit_confidence = hit_confidence, lowest_conc = lowest_conc) # To merge with other sets
simulated_curves_Dark_2_act_set <- Dark_2_out$result$act_set

simulated_curves_Dark_2_summary <- simulated_curves_Dark_2 %>%
  inner_join(simulated_curves_Dark_2_act_set) %>%
  group_by(chemical, conc, sample_id) %>%
  summarise(endpoint = endpoint, resp = resp, corrected_resp = corrected_resp, correction = correction, POD = POD, highest_conc = highest_conc, hit = hit, lowest_conc = lowest_conc) %>%
  inner_join(simulated_curve_bmds_Dark_2) %>%
  mutate(resp = if_else(correction == 1, true = corrected_resp, false = resp)) %>%
  select(-corrected_resp) %>%
  mutate(median_POD = if_else(condition = POD == lowest_conc, true = unique(highest_conc), false = POD)) %>%
  mutate(median_POD = if_else(condition = is.na(median(median_POD, na.rm = TRUE)), true = unique(highest_conc), false = median(median_POD, na.rm = TRUE))) %>%
...
confident_curves_Dark_2 <- curve_hits_Dark_2%>%
  filter(hit_confidence >= 0.5) %>%
  group_by(chemical, sample_id) %>%
  mutate(POD_diff_med = abs(POD-POD_med), POD_diff_median = abs(POD-median_POD), POD_diff_mean = abs(POD-mean_POD), smallest_diff = min(c(POD_diff_med, POD_diff_mean, POD_diff_median)), conf_POD = if_else(condition = smallest_diff == POD_diff_med, true = "med", false = if_else(condition = smallest_diff == POD_diff_median, true = "median", false = "mean"))) %>%
  ungroup() %>%
...
## Dark_3
simulated_curves_Dark_3 <- Dark_3_out$result$resp_set # To merge with other sets
simulated_curve_bmds_Dark_3 <- Dark_3_out$act_summary %>%
  group_by(chemical) %>%
  summarise(POD_med = POD_med, POD_cil = POD_cil, hit_confidence = hit_confidence, lowest_conc = lowest_conc) # To merge with other sets
simulated_curves_Dark_3_act_set <- Dark_3_out$result$act_set

simulated_curves_Dark_3_summary <- simulated_curves_Dark_3 %>%
  inner_join(simulated_curves_Dark_3_act_set) %>%
  group_by(chemical, conc, sample_id) %>%
  summarise(endpoint = endpoint, resp = resp, corrected_resp = corrected_resp, correction = correction, POD = POD, highest_conc = highest_conc, hit = hit, lowest_conc = lowest_conc) %>%
  inner_join(simulated_curve_bmds_Dark_3) %>%
  mutate(resp = if_else(correction == 1, true = corrected_resp, false = resp)) %>%
  select(-corrected_resp) %>%
  mutate(median_POD = if_else(condition = POD == lowest_conc, true = unique(highest_conc), false = POD)) %>%
  mutate(median_POD = if_else(condition = is.na(median(median_POD, na.rm = TRUE)), true = unique(highest_conc), false = median(median_POD, na.rm = TRUE))) %>%
...
confident_curves_Dark_3 <- curve_hits_Dark_3%>%
  filter(hit_confidence >= 0.5) %>%
  group_by(chemical, sample_id) %>%
  mutate(POD_diff_med = abs(POD-POD_med), POD_diff_median = abs(POD-median_POD), POD_diff_mean = abs(POD-mean_POD), smallest_diff = min(c(POD_diff_med, POD_diff_mean, POD_diff_median)), conf_POD = if_else(condition = smallest_diff == POD_diff_med, true = "med", false = if_else(condition = smallest_diff == POD_diff_median, true = "median", false = "mean"))) %>%
  ungroup() %>%
...
## Light
simulated_curves_Light <- Light_out$result$resp_set # To merge with other sets
simulated_curve_bmds_Light <- Light_out$act_summary %>%
  group_by(chemical) %>%
  summarise(POD_med = POD_med, POD_cil = POD_cil, hit_confidence = hit_confidence, lowest_conc = lowest_conc) # To merge with other sets
simulated_curves_Light_act_set <- Light_out$result$act_set

simulated_curves_Light_summary <- simulated_curves_Light %>%
  inner_join(simulated_curves_Light_act_set) %>%
  group_by(chemical, conc, sample_id) %>%
  summarise(endpoint = endpoint, resp = resp, corrected_resp = corrected_resp, correction = correction, POD = POD, highest_conc = highest_conc, hit = hit, lowest_conc = lowest_conc) %>%
  inner_join(simulated_curve_bmds_Light) %>%
  mutate(resp = if_else(correction == 1, true = corrected_resp, false = resp)) %>%
  select(-corrected_resp) %>%
  mutate(median_POD = if_else(condition = POD == lowest_conc, true = unique(highest_conc), false = POD)) %>%
  mutate(median_POD = if_else(condition = is.na(median(median_POD, na.rm = TRUE)), true = unique(highest_conc), false = median(median_POD, na.rm = TRUE))) %>%
...
confident_curves_Light <- curve_hits_Light%>%
  filter(hit_confidence >= 0.5) %>%
  group_by(chemical, sample_id) %>%
  mutate(POD_diff_med = abs(POD-POD_med), POD_diff_median = abs(POD-median_POD), POD_diff_mean = abs(POD-mean_POD), smallest_diff = min(c(POD_diff_med, POD_diff_mean, POD_diff_median)), conf_POD = if_else(condition = smallest_diff == POD_diff_med, true = "med", false = if_else(condition = smallest_diff == POD_diff_median, true = "median", false = "mean"))) %>%
  ungroup() %>%
...
## Dark
simulated_curves_Dark <- Dark_out$result$resp_set # To merge with other sets
simulated_curve_bmds_Dark <- Dark_out$act_summary %>%
  group_by(chemical) %>%
  summarise(POD_med = POD_med, POD_cil = POD_cil, hit_confidence = hit_confidence, lowest_conc = lowest_conc) # To merge with other sets
simulated_curves_Dark_act_set <- Dark_out$result$act_set

simulated_curves_Dark_summary <- simulated_curves_Dark %>%
  inner_join(simulated_curves_Dark_act_set) %>%
  group_by(chemical, conc, sample_id) %>%
  summarise(endpoint = endpoint, resp = resp, corrected_resp = corrected_resp, correction = correction, POD = POD, highest_conc = highest_conc, hit = hit, lowest_conc = lowest_conc) %>%
  inner_join(simulated_curve_bmds_Dark) %>%
  mutate(resp = if_else(correction == 1, true = corrected_resp, false = resp)) %>%
  select(-corrected_resp) %>%
  mutate(median_POD = if_else(condition = POD == lowest_conc, true = unique(highest_conc), false = POD)) %>%
  mutate(median_POD = if_else(condition = is.na(median(median_POD, na.rm = TRUE)), true = unique(highest_conc), false = median(median_POD, na.rm = TRUE))) %>%
...
confident_curves_Dark <- curve_hits_Dark%>%
  filter(hit_confidence >= 0.5) %>%
  group_by(chemical, sample_id) %>%
  mutate(POD_diff_med = abs(POD-POD_med), POD_diff_median = abs(POD-median_POD), POD_diff_mean = abs(POD-mean_POD), smallest_diff = min(c(POD_diff_med, POD_diff_mean, POD_diff_median)), conf_POD = if_else(condition = smallest_diff == POD_diff_med, true = "med", false = if_else(condition = smallest_diff == POD_diff_median, true = "median", false = "mean"))) %>%
  ungroup() %>%
...
```{r 96curve_simulation_auc, include = FALSE}
## Light 1
p_Light_1 <- ggplot(NULL, aes(conc, resp)) +
  ...
  geom_line(data = confident_curves_Light_1[[1]], mapping = aes(x = conc, y = resp, group = sample_id), color = "yellow") +
  # geom_line(data = confident_curves_Light_1[[2]], mapping = aes(x = conc, y = resp, group = sample_id), color = "yellow") +
  ...
  geom_text(data = simulated_curve_bmds_Light_1, mapping = aes(label = paste("Hit =", hit_confidence), x = lowest_conc+1), y = -85, inherit.aes = FALSE, size = 3) +
  facet_wrap(~chemical, scales = "free") +
  ylim(-100,50) +
  theme_minimal() +
  theme(panel.border = element_rect(colour = "grey50", fill = NA), panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
p_Light_2 <- ggplot(NULL, aes(conc, resp)) +
  ...
  geom_text(data = simulated_curve_bmds_Light_2, mapping = aes(label = paste("Hit =", hit_confidence), x = lowest_conc+1), y = -85, inherit.aes = FALSE, size = 3) +
  facet_wrap(~chemical, scales = "free") +
  ylim(-100,50) +
  theme_minimal() +
  theme(panel.border = element_rect(colour = "grey50", fill = NA), panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
p_Light_3 <- ggplot(NULL, aes(conc, resp)) +
  ...
  geom_text(data = simulated_curve_bmds_Light_3, mapping = aes(label = paste("Hit =", hit_confidence), x = lowest_conc+1), y = -85, inherit.aes = FALSE, size = 3) +
  facet_wrap(~chemical, scales = "free") +
  ylim(-100,50) +
  theme_minimal() +
  theme(panel.border = element_rect(colour = "grey50", fill = NA), panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
p_Dark_1 <- ggplot(NULL, aes(conc, resp)) +
  ...
  geom_text(data = simulated_curve_bmds_Dark_1, mapping = aes(label = paste("Hit =", hit_confidence), x = lowest_conc+1), y = -85, inherit.aes = FALSE, size = 3) +
  facet_wrap(~chemical, scales = "free") +
  ylim(-100,50) +
  theme_minimal() +
  theme(panel.border = element_rect(colour = "grey50", fill = NA), panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
p_Dark_2 <- ggplot(NULL, aes(conc, resp)) +
  ...
  geom_text(data = simulated_curve_bmds_Dark_2, mapping = aes(label = paste("Hit =", hit_confidence), x = lowest_conc+1), y = -85, inherit.aes = FALSE, size = 3) +
  facet_wrap(~chemical, scales = "free") +
  ylim(-100,50) +
  theme_minimal() +
  theme(panel.border = element_rect(colour = "grey50", fill = NA), panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
p_Dark_3 <- ggplot(NULL, aes(conc, resp)) +
  ...
  geom_text(data = simulated_curve_bmds_Dark_3, mapping = aes(label = paste("Hit =", hit_confidence), x = lowest_conc+1), y = -85, inherit.aes = FALSE, size = 3) +
  facet_wrap(~chemical, scales = "free") +
  ylim(-100,50) +
  theme_minimal() +
  theme(panel.border = element_rect(colour = "grey50", fill = NA), panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
p_Light <- ggplot(NULL, aes(conc, resp)) +
  ...
  geom_text(data = simulated_curve_bmds_Light, mapping = aes(label = paste("Hit =", hit_confidence), x = lowest_conc+1), y = -85, inherit.aes = FALSE, size = 3) +
  facet_wrap(~chemical, scales = "free") +
  ylim(-100,50) +
  theme_minimal() +
  theme(panel.border = element_rect(colour = "grey50", fill = NA), panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
p_Dark <- ggplot(NULL, aes(conc, resp)) +
  ...
  geom_text(data = simulated_curve_bmds_Dark, mapping = aes(label = paste("Hit =", hit_confidence), x = lowest_conc+1), y = -85, inherit.aes = FALSE, size = 3) +
  facet_wrap(~chemical, scales = "free") +
  ylim(-100,50) +
  theme_minimal() +
  theme(panel.border = element_rect(colour = "grey50", fill = NA), panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
## Light_1
Light_1_fitted <- simulated_curves_Light_1_summary %>%
  select(endpoint, chemical, conc, resp) %>%
  group_by(chemical) %>%
  run_fit(
  .,
...
## Light_2
Light_2_fitted <- simulated_curves_Light_2_summary %>%
  select(endpoint, chemical, conc, resp) %>%
  group_by(chemical) %>%
  run_fit(
  .,
...
## Light_3
Light_3_fitted <- simulated_curves_Light_3_summary %>%
  select(endpoint, chemical, conc, resp) %>%
  group_by(chemical) %>%
  run_fit(
  .,
...
## Dark_1
Dark_1_fitted <- simulated_curves_Dark_1_summary %>%
  select(endpoint, chemical, conc, resp) %>%
  group_by(chemical) %>%
  run_fit(
  .,
...
## Dark_2
Dark_2_fitted <- simulated_curves_Dark_2_summary %>%
  select(endpoint, chemical, conc, resp) %>%
  group_by(chemical) %>%
  run_fit(
  .,
...
## Dark_3
Dark_3_fitted <- simulated_curves_Dark_3_summary %>%
  select(endpoint, chemical, conc, resp) %>%
  group_by(chemical) %>%
  run_fit(
  .,
...
## Light
Light_fitted <- simulated_curves_Light_summary %>%
  select(endpoint, chemical, conc, resp) %>%
  group_by(chemical) %>%
  run_fit(
  .,
...
## Dark
Dark_fitted <- simulated_curves_Dark_summary %>%
  select(endpoint, chemical, conc, resp) %>%
  group_by(chemical) %>%
  run_fit(
  .,
...
Light_1_fitted_output_1 <- Light_1_fitted_output$result$resp_set %>%
  inner_join(Light_1_fitted_output$result$act_set) %>%
  group_by(chemical, conc, sample_id) %>%
  summarise(fitted_resp = fitted_resp, POD = POD, lowest_conc = lowest_conc, highest_conc = highest_conc, endpoint = endpoint, hit = hit) %>%
  inner_join(Light_1_fitted_output$act_summary) %>%
  select(endpoint, chemical, sample_id, conc, fitted_resp, lowest_conc, highest_conc, POD, POD_med, POD_cil, POD_ciu, hit, hit_confidence)

Light_1_fitted_output_summary <- Light_1_fitted_output_1 %>%
  group_by(chemical, conc, sample_id) %>%
  summarise(fitted_resp = unique(fitted_resp), lowest_conc = unique(lowest_conc), highest_conc = unique(highest_conc), POD = unique(POD), POD_cil = unique(POD_cil), POD_med = unique(POD_med), POD_ciu = unique(POD_ciu), hit = unique(hit), hit_confidence = unique(hit_confidence)) %>%
    mutate(median_POD = if_else(condition = POD == lowest_conc, true = unique(highest_conc), false = POD)) %>%
  mutate(median_POD = if_else(condition = is.na(median(median_POD, na.rm = TRUE)), true = unique(highest_conc), false = median(median_POD, na.rm = TRUE))) %>%
...
fitted_hits_Light_1 <- Light_1_fitted_output_summary %>%
  filter(hit == 1)

confident_fitted_hits_Light_1 <- fitted_hits_Light_1 %>%
  filter(hit_confidence >= 0.5) %>%
  group_by(chemical, sample_id) %>%
  mutate(POD_diff_med = abs(POD-POD_med), POD_diff_median = abs(POD-median_POD), POD_diff_mean = abs(POD-mean_POD), smallest_diff = min(c(POD_diff_med, POD_diff_mean, POD_diff_median)), conf_POD = if_else(condition = smallest_diff == POD_diff_med, true = "med", false = if_else(condition = smallest_diff == POD_diff_median, true = "median", false = "mean"))) %>%
  ungroup() %>%
...
Light_2_fitted_output_1 <- Light_2_fitted_output$result$resp_set %>%
  inner_join(Light_2_fitted_output$result$act_set) %>%
  group_by(chemical, conc, sample_id) %>%
  summarise(fitted_resp = fitted_resp, POD = POD, lowest_conc = lowest_conc, highest_conc = highest_conc, endpoint = endpoint, hit = hit) %>%
  inner_join(Light_2_fitted_output$act_summary) %>%
  select(endpoint, chemical, sample_id, conc, fitted_resp, lowest_conc, highest_conc, POD, POD_med, POD_cil, POD_ciu, hit, hit_confidence)

Light_2_fitted_output_summary <- Light_2_fitted_output_1 %>%
  group_by(chemical, conc, sample_id) %>%
  summarise(fitted_resp = unique(fitted_resp), lowest_conc = unique(lowest_conc), highest_conc = unique(highest_conc), POD = unique(POD), POD_cil = unique(POD_cil), POD_med = unique(POD_med), POD_ciu = unique(POD_ciu), hit = unique(hit), hit_confidence = unique(hit_confidence)) %>%
    mutate(median_POD = if_else(condition = POD == lowest_conc, true = unique(highest_conc), false = POD)) %>%
  mutate(median_POD = if_else(condition = is.na(median(median_POD, na.rm = TRUE)), true = unique(highest_conc), false = median(median_POD, na.rm = TRUE))) %>%
...
fitted_hits_Light_2 <- Light_2_fitted_output_summary %>%
  filter(hit == 1)

confident_fitted_hits_Light_2 <- fitted_hits_Light_2 %>%
  filter(hit_confidence >= 0.5) %>%
  group_by(chemical, sample_id) %>%
  mutate(POD_diff_med = abs(POD-POD_med), POD_diff_median = abs(POD-median_POD), POD_diff_mean = abs(POD-mean_POD), smallest_diff = min(c(POD_diff_med, POD_diff_mean, POD_diff_median)), conf_POD = if_else(condition = smallest_diff == POD_diff_med, true = "med", false = if_else(condition = smallest_diff == POD_diff_median, true = "median", false = "mean"))) %>%
  ungroup() %>%
...
Light_3_fitted_output_1 <- Light_3_fitted_output$result$resp_set %>%
  inner_join(Light_3_fitted_output$result$act_set) %>%
  group_by(chemical, conc, sample_id) %>%
  summarise(fitted_resp = fitted_resp, POD = POD, lowest_conc = lowest_conc, highest_conc = highest_conc, endpoint = endpoint, hit = hit) %>%
  inner_join(Light_3_fitted_output$act_summary) %>%
  select(endpoint, chemical, sample_id, conc, fitted_resp, lowest_conc, highest_conc, POD, POD_med, POD_cil, POD_ciu, hit, hit_confidence)

Light_3_fitted_output_summary <- Light_3_fitted_output_1 %>%
  group_by(chemical, conc, sample_id) %>%
  summarise(fitted_resp = unique(fitted_resp), lowest_conc = unique(lowest_conc), highest_conc = unique(highest_conc), POD = unique(POD), POD_cil = unique(POD_cil), POD_med = unique(POD_med), POD_ciu = unique(POD_ciu), hit = unique(hit), hit_confidence = unique(hit_confidence)) %>%
    mutate(median_POD = if_else(condition = POD == lowest_conc, true = unique(highest_conc), false = POD)) %>%
  mutate(median_POD = if_else(condition = is.na(median(median_POD, na.rm = TRUE)), true = unique(highest_conc), false = median(median_POD, na.rm = TRUE))) %>%
...
fitted_hits_Light_3 <- Light_3_fitted_output_summary %>%
  filter(hit == 1)

confident_fitted_hits_Light_3 <- fitted_hits_Light_3 %>%
  filter(hit_confidence >= 0.5) %>%
  group_by(chemical, sample_id) %>%
  mutate(POD_diff_med = abs(POD-POD_med), POD_diff_median = abs(POD-median_POD), POD_diff_mean = abs(POD-mean_POD), smallest_diff = min(c(POD_diff_med, POD_diff_mean, POD_diff_median)), conf_POD = if_else(condition = smallest_diff == POD_diff_med, true = "med", false = if_else(condition = smallest_diff == POD_diff_median, true = "median", false = "mean"))) %>%
  ungroup() %>%
...
Dark_1_fitted_output_1 <- Dark_1_fitted_output$result$resp_set %>%
  inner_join(Dark_1_fitted_output$result$act_set) %>%
  group_by(chemical, conc, sample_id) %>%
  summarise(fitted_resp = fitted_resp, POD = POD, lowest_conc = lowest_conc, highest_conc = highest_conc, endpoint = endpoint, hit = hit) %>%
  inner_join(Dark_1_fitted_output$act_summary) %>%
  select(endpoint, chemical, sample_id, conc, fitted_resp, lowest_conc, highest_conc, POD, POD_med, POD_cil, POD_ciu, hit, hit_confidence)

Dark_1_fitted_output_summary <- Dark_1_fitted_output_1 %>%
  group_by(chemical, conc, sample_id) %>%
  summarise(fitted_resp = unique(fitted_resp), lowest_conc = unique(lowest_conc), highest_conc = unique(highest_conc), POD = unique(POD), POD_cil = unique(POD_cil), POD_med = unique(POD_med), POD_ciu = unique(POD_ciu), hit = unique(hit), hit_confidence = unique(hit_confidence)) %>%
    mutate(median_POD = if_else(condition = POD == lowest_conc, true = unique(highest_conc), false = POD)) %>%
  mutate(median_POD = if_else(condition = is.na(median(median_POD, na.rm = TRUE)), true = unique(highest_conc), false = median(median_POD, na.rm = TRUE))) %>%
...
fitted_hits_Dark_1 <- Dark_1_fitted_output_summary %>%
  filter(hit == 1)

confident_fitted_hits_Dark_1 <- fitted_hits_Dark_1 %>%
  filter(hit_confidence >= 0.5) %>%
  group_by(chemical, sample_id) %>%
  mutate(POD_diff_med = abs(POD-POD_med), POD_diff_median = abs(POD-median_POD), POD_diff_mean = abs(POD-mean_POD), smallest_diff = min(c(POD_diff_med, POD_diff_mean, POD_diff_median)), conf_POD = if_else(condition = smallest_diff == POD_diff_med, true = "med", false = if_else(condition = smallest_diff == POD_diff_median, true = "median", false = "mean"))) %>%
  ungroup() %>%
...
Dark_2_fitted_output_1 <- Dark_2_fitted_output$result$resp_set %>%
  inner_join(Dark_2_fitted_output$result$act_set) %>%
  group_by(chemical, conc, sample_id) %>%
  summarise(fitted_resp = fitted_resp, POD = POD, lowest_conc = lowest_conc, highest_conc = highest_conc, endpoint = endpoint, hit = hit) %>%
  inner_join(Dark_2_fitted_output$act_summary) %>%
  select(endpoint, chemical, sample_id, conc, fitted_resp, lowest_conc, highest_conc, POD, POD_med, POD_cil, POD_ciu, hit, hit_confidence)

Dark_2_fitted_output_summary <- Dark_2_fitted_output_1 %>%
  group_by(chemical, conc, sample_id) %>%
  summarise(fitted_resp = unique(fitted_resp), lowest_conc = unique(lowest_conc), highest_conc = unique(highest_conc), POD = unique(POD), POD_cil = unique(POD_cil), POD_med = unique(POD_med), POD_ciu = unique(POD_ciu), hit = unique(hit), hit_confidence = unique(hit_confidence)) %>%
    mutate(median_POD = if_else(condition = POD == lowest_conc, true = unique(highest_conc), false = POD)) %>%
  mutate(median_POD = if_else(condition = is.na(median(median_POD, na.rm = TRUE)), true = unique(highest_conc), false = median(median_POD, na.rm = TRUE))) %>%
...
fitted_hits_Dark_2 <- Dark_2_fitted_output_summary %>%
  filter(hit == 1)

confident_fitted_hits_Dark_2 <- fitted_hits_Dark_2 %>%
  filter(hit_confidence >= 0.5) %>%
  group_by(chemical, sample_id) %>%
  mutate(POD_diff_med = abs(POD-POD_med), POD_diff_median = abs(POD-median_POD), POD_diff_mean = abs(POD-mean_POD), smallest_diff = min(c(POD_diff_med, POD_diff_mean, POD_diff_median)), conf_POD = if_else(condition = smallest_diff == POD_diff_med, true = "med", false = if_else(condition = smallest_diff == POD_diff_median, true = "median", false = "mean"))) %>%
  ungroup() %>%
...
Dark_3_fitted_output_1 <- Dark_3_fitted_output$result$resp_set %>%
  inner_join(Dark_3_fitted_output$result$act_set) %>%
  group_by(chemical, conc, sample_id) %>%
  summarise(fitted_resp = fitted_resp, POD = POD, lowest_conc = lowest_conc, highest_conc = highest_conc, endpoint = endpoint, hit = hit) %>%
  inner_join(Dark_3_fitted_output$act_summary) %>%
  select(endpoint, chemical, sample_id, conc, fitted_resp, lowest_conc, highest_conc, POD, POD_med, POD_cil, POD_ciu, hit, hit_confidence)

Dark_3_fitted_output_summary <- Dark_3_fitted_output_1 %>%
  group_by(chemical, conc, sample_id) %>%
  summarise(fitted_resp = unique(fitted_resp), lowest_conc = unique(lowest_conc), highest_conc = unique(highest_conc), POD = unique(POD), POD_cil = unique(POD_cil), POD_med = unique(POD_med), POD_ciu = unique(POD_ciu), hit = unique(hit), hit_confidence = unique(hit_confidence)) %>%
    mutate(median_POD = if_else(condition = POD == lowest_conc, true = unique(highest_conc), false = POD)) %>%
  mutate(median_POD = if_else(condition = is.na(median(median_POD, na.rm = TRUE)), true = unique(highest_conc), false = median(median_POD, na.rm = TRUE))) %>%
...
fitted_hits_Dark_3 <- Dark_3_fitted_output_summary %>%
  filter(hit == 1)

confident_fitted_hits_Dark_3 <- fitted_hits_Dark_3 %>%
  filter(hit_confidence >= 0.5) %>%
  group_by(chemical, sample_id) %>%
  mutate(POD_diff_med = abs(POD-POD_med), POD_diff_median = abs(POD-median_POD), POD_diff_mean = abs(POD-mean_POD), smallest_diff = min(c(POD_diff_med, POD_diff_mean, POD_diff_median)), conf_POD = if_else(condition = smallest_diff == POD_diff_med, true = "med", false = if_else(condition = smallest_diff == POD_diff_median, true = "median", false = "mean"))) %>%
  ungroup() %>%
...
Light_fitted_output_1 <- Light_fitted_output$result$resp_set %>%
  inner_join(Light_fitted_output$result$act_set) %>%
  group_by(chemical, conc, sample_id) %>%
  summarise(fitted_resp = fitted_resp, POD = POD, lowest_conc = lowest_conc, highest_conc = highest_conc, endpoint = endpoint, hit = hit) %>%
  inner_join(Light_fitted_output$act_summary) %>%
  select(endpoint, chemical, sample_id, conc, fitted_resp, lowest_conc, highest_conc, POD, POD_med, POD_cil, POD_ciu, hit, hit_confidence)

Light_fitted_output_summary <- Light_fitted_output_1 %>%
  group_by(chemical, conc, sample_id) %>%
  summarise(fitted_resp = unique(fitted_resp), lowest_conc = unique(lowest_conc), highest_conc = unique(highest_conc), POD = unique(POD), POD_cil = unique(POD_cil), POD_med = unique(POD_med), POD_ciu = unique(POD_ciu), hit = unique(hit), hit_confidence = unique(hit_confidence)) %>%
    mutate(median_POD = if_else(condition = POD == lowest_conc, true = unique(highest_conc), false = POD)) %>%
  mutate(median_POD = if_else(condition = is.na(median(median_POD, na.rm = TRUE)), true = unique(highest_conc), false = median(median_POD, na.rm = TRUE))) %>%
...
fitted_hits_Light <- Light_fitted_output_summary %>%
  filter(hit == 1)

confident_fitted_hits_Light <- fitted_hits_Light %>%
  filter(hit_confidence >= 0.5) %>%
  group_by(chemical, sample_id) %>%
  mutate(POD_diff_med = abs(POD-POD_med), POD_diff_median = abs(POD-median_POD), POD_diff_mean = abs(POD-mean_POD), smallest_diff = min(c(POD_diff_med, POD_diff_mean, POD_diff_median)), conf_POD = if_else(condition = smallest_diff == POD_diff_med, true = "med", false = if_else(condition = smallest_diff == POD_diff_median, true = "median", false = "mean"))) %>%
  ungroup() %>%
...
Dark_fitted_output_1 <- Dark_fitted_output$result$resp_set %>%
  inner_join(Dark_fitted_output$result$act_set) %>%
  group_by(chemical, conc, sample_id) %>%
  summarise(fitted_resp = fitted_resp, POD = POD, lowest_conc = lowest_conc, highest_conc = highest_conc, endpoint = endpoint, hit = hit) %>%
  inner_join(Dark_fitted_output$act_summary) %>%
  select(endpoint, chemical, sample_id, conc, fitted_resp, lowest_conc, highest_conc, POD, POD_med, POD_cil, POD_ciu, hit, hit_confidence)

Dark_fitted_output_summary <- Dark_fitted_output_1 %>%
  group_by(chemical, conc, sample_id) %>%
  summarise(fitted_resp = unique(fitted_resp), lowest_conc = unique(lowest_conc), highest_conc = unique(highest_conc), POD = unique(POD), POD_cil = unique(POD_cil), POD_med = unique(POD_med), POD_ciu = unique(POD_ciu), hit = unique(hit), hit_confidence = unique(hit_confidence)) %>%
    mutate(median_POD = if_else(condition = POD == lowest_conc, true = unique(highest_conc), false = POD)) %>%
  mutate(median_POD = if_else(condition = is.na(median(median_POD, na.rm = TRUE)), true = unique(highest_conc), false = median(median_POD, na.rm = TRUE))) %>%
...
fitted_hits_Dark <- Dark_fitted_output_summary %>%
  filter(hit == 1)

confident_fitted_hits_Dark <- fitted_hits_Dark %>%
  filter(hit_confidence >= 0.5) %>%
  group_by(chemical, sample_id) %>%
  mutate(POD_diff_med = abs(POD-POD_med), POD_diff_median = abs(POD-median_POD), POD_diff_mean = abs(POD-mean_POD), smallest_diff = min(c(POD_diff_med, POD_diff_mean, POD_diff_median)), conf_POD = if_else(condition = smallest_diff == POD_diff_med, true = "med", false = if_else(condition = smallest_diff == POD_diff_median, true = "median", false = "mean"))) %>%
  ungroup() %>%
...
p_Light_1_fitted <- ggplot(NULL, aes(conc, resp)) +
  ...
  geom_text(data = Light_1_fitted_output_summary, mapping = aes(label = paste("Hit =", hit_confidence), x = lowest_conc+1), y = -60, inherit.aes = FALSE, size = 3) +
  facet_wrap(~chemical, scales = "free_x") +
  ylim(-100,50) +
  theme_minimal() +
  theme(panel.border = element_rect(colour = "grey50", fill = NA))
...
p_Light_2_fitted <- ggplot(NULL, aes(conc, resp)) +
  ...
  geom_line(data = fitted_hits_Light_2, mapping = aes(x = conc, y = fitted_resp, group = sample_id), color = "#a9a9a9",  alpha = 0.7) +
  ...
  geom_text(data = Light_2_fitted_output_summary, mapping = aes(label = paste("Hit =", hit_confidence), x = lowest_conc+1), y = -60, inherit.aes = FALSE, size = 3) +
  facet_wrap(~chemical, scales = "free_x") +
  ylim(-100,50) +
  theme_minimal() +
  theme(panel.border = element_rect(colour = "grey50", fill = NA))
...
p_Light_3_fitted <- ggplot(NULL, aes(conc, resp)) +
  ...
  geom_line(data = fitted_hits_Light_3, mapping = aes(x = conc, y = fitted_resp, group = sample_id), color = "#a9a9a9",  alpha = 0.7) +
  ...
  geom_text(data = Light_3_fitted_output_summary, mapping = aes(label = paste("Hit =", hit_confidence), x = lowest_conc+1), y = -60, inherit.aes = FALSE, size = 3) +
  facet_wrap(~chemical, scales = "free_x") +
  ylim(-100,50) +
  theme_minimal() +
  theme(panel.border = element_rect(colour = "grey50", fill = NA))
...
p_Dark_1_fitted <- ggplot(NULL, aes(conc, resp)) +
  ...
  geom_line(data = fitted_hits_Dark_1, mapping = aes(x = conc, y = fitted_resp, group = sample_id), color = "#a9a9a9",  alpha = 0.7) +
  ...
  geom_text(data = Dark_1_fitted_output_summary, mapping = aes(label = paste("Hit =", hit_confidence), x = lowest_conc+1), y = -60, inherit.aes = FALSE, size = 3) +
  facet_wrap(~chemical, scales = "free_x") +
  ylim(-100,50) +
  theme_minimal() +
  theme(panel.border = element_rect(colour = "grey50", fill = NA))
...
p_Dark_2_fitted <- ggplot(NULL, aes(conc, resp)) +
  ...
  geom_line(data = fitted_hits_Dark_2, mapping = aes(x = conc, y = fitted_resp, group = sample_id), color = "#a9a9a9",  alpha = 0.7) +
  ...
  geom_text(data = Dark_2_fitted_output_summary, mapping = aes(label = paste("Hit =", hit_confidence), x = lowest_conc+1), y = -60, inherit.aes = FALSE, size = 3) +
  facet_wrap(~chemical, scales = "free_x") +
  ylim(-100,50) +
  theme_minimal() +
  theme(panel.border = element_rect(colour = "grey50", fill = NA))
...
p_Dark_3_fitted <- ggplot(NULL, aes(conc, resp)) +
  ...
  geom_line(data = fitted_hits_Dark_3, mapping = aes(x = conc, y = fitted_resp, group = sample_id), color = "#a9a9a9",  alpha = 0.7) +
  ...
  geom_text(data = Dark_3_fitted_output_summary, mapping = aes(label = paste("Hit =", hit_confidence), x = lowest_conc+1), y = -60, inherit.aes = FALSE, size = 3) +
  facet_wrap(~chemical, scales = "free_x") +
  ylim(-100,50) +
  theme_minimal() +
  theme(panel.border = element_rect(colour = "grey50", fill = NA))
...
p_Light_fitted <- ggplot(NULL, aes(conc, resp)) +
  ...
  geom_line(data = fitted_hits_Light, mapping = aes(x = conc, y = fitted_resp, group = sample_id), color = "#a9a9a9",  alpha = 0.7) +
  ...
  geom_text(data = Light_fitted_output_summary, mapping = aes(label = paste("Hit =", hit_confidence), x = lowest_conc+1), y = -60, inherit.aes = FALSE, size = 3) +
  facet_wrap(~chemical, scales = "free_x") +
  ylim(-100,50) +
  theme_minimal() +
  theme(panel.border = element_rect(colour = "grey50", fill = NA))
...
p_Dark_fitted <- ggplot(NULL, aes(conc, resp)) +
  ...
  geom_line(data = fitted_hits_Dark, mapping = aes(x = conc, y = fitted_resp, group = sample_id), color = "#a9a9a9",  alpha = 0.7) +
  ...
  geom_text(data = Dark_fitted_output_summary, mapping = aes(label = paste("Hit =", hit_confidence), x = lowest_conc+1), y = -60, inherit.aes = FALSE, size = 3) +
  facet_wrap(~chemical, scales = "free_x") +
  ylim(-100,50) +
  theme_minimal() +
  theme(panel.border = element_rect(colour = "grey50", fill = NA))
