...
curve_hits_pearson %>%
  group_by(chemical) %>%
  mutate(median_POD = median(POD), low_conf_POD = quantile(POD, c(0.05)), high_conf_POD = quantile(POD, c(0.95))) %>%
ggplot(data = ., aes(x = POD)) +
  geom_histogram() +
  facet_wrap(~ chemical, scales = "free") +
  geom_vline(aes(xintercept = median_POD), color = "blue") +
  geom_vline(aes(xintercept = low_conf_POD), color = "green") +
  geom_vline(aes(xintercept = high_conf_POD), color = "purple") +
  geom_vline(aes(xintercept = POD_med), color = "yellow") +
  geom_vline(aes(xintercept = POD_cil), color = "red")
  # geom_vline(aes(xintercept = POD_ciu), color = "purple")
