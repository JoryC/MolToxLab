...
confident_curves_spearman <- curve_hits_spearman %>%
  filter(POD == median_POD | POD == POD_med, hit_confidence >= 0.5)
...
p_spearman <- ggplot(NULL, aes(conc, resp)) +
  ...
  geom_line(data = confident_curves_spearman, mapping = aes(x = conc, y = resp, group = sample_id), color = "black") +
...
# pearson_fitted <- simulated_curves_pearson %>%
#   group_by(chemical) %>%
#   run_fit(
#   .,
#   modls = c("hill", "cnst"),
#   keep_sets = c("fit_set", "resp_set"), 
#   hill_pdir = -1, #Negative direction only
#   n_samples = params$sample_size
# )
# 
# pearson_fitted_output <- summarize_fit_output(d = pearson_fitted, thr_resp = bmr_thresh_pearson)
# 
# pearson_fitted_output_summary <- pearson_fitted_output$result$resp_set %>%
#   group_by(chemical, sample_id, conc) %>%
#   summarize(fitted_resp = unique(na.omit(fitted_resp)))
# 
# p_pearson_fitted <- ggplot(data = pearson_fitted_output_summary, aes(x = conc, y = fitted_resp)) +
#   geom_smooth(se = FALSE) +
#   geom_vline(data = pearson_fitted_output$act_summary, aes(xintercept = POD_med), color = "blue") +
#   geom_vline(data = pearson_fitted_output$act_summary, aes(xintercept = POD_cil), color = "green") +
#   geom_hline(yintercept = -bmr_thresh_pearson, linetype = "dashed", color = "red") +
...
spearman_fitted_output_summary <- spearman_fitted_output$result$resp_set %>%
  group_by(chemical, sample_id, conc) %>%
  summarise(fitted_resp = na.omit(fitted_resp))

p_spearman_fitted <- ggplot(data = NULL, aes(x = conc, y = fitted_resp)) +
  geom_line(data = spearman_fitted_output_summary, aes(x = conc, y = fitted_resp, group = sample_id), color = "gray75") +
  geom_smooth(data = spearman_fitted_output_summary, aes(x = conc, y = fitted_resp)) +
  geom_point(data = na.omit(spearman_fitted_output$result$resp_set), aes(y = resp)) +
  geom_vline(data = spearman_fitted_output$act_summary, aes(xintercept = POD_med), color = "blue") +
  geom_vline(data = spearman_fitted_output$act_summary, aes(xintercept = POD_cil), color = "green") +
  geom_hline(yintercept = -bmr_thresh_spearman, linetype = "dashed", color = "red") +
  geom_hline(yintercept = bmr_thresh_spearman, linetype = "dashed", color = "red") +
  geom_text(data = spearman_fitted_output$act_summary, aes(label = paste("Hit =", hit_confidence), x = lowest_conc + 1), y = -100) +
  facet_wrap(~chemical, scales = "free_x") +
  theme_minimal() +
  theme(panel.border = element_rect(colour = "grey50", fill = NA))
