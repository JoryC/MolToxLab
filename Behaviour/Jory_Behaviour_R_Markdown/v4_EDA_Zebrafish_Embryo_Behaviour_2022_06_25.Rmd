...
From the sampling method, it seems that the benchmark response threshold should be set to `r bmr_thresh_pearson` for the pearson endpoint and `r bmr_thresh_spearman` for the spearman endpoint, but it is advised to be cautious of doing so. In the future, there will be a visualization of all of the simulated curves to try to visually determine if the BMR is appropriate.
...
sum_bmd_dat_nobin_neg_pearson <- summarize_rcurvep_output(bmd_dat_nobin_neg_pearson)
sum_bmd_dat_nobin_neg_spearman <- summarize_rcurvep_output(bmd_dat_nobin_neg_spearman)
...
simulated_curves_pearson <- sum_bmd_dat_nobin_neg_pearson$result$resp_set # To merge with other sets

simulated_curve_bmds_pearson <- sum_bmd_dat_nobin_neg_pearson$act_summary %>%
  group_by(chemical) %>%
  summarise(POD_med = POD_med, POD_cil = POD_cil, hit_confidence = hit_confidence, lowest_conc = lowest_conc) # To merge with other sets

simulated_curves_pearson_act_set <- sum_bmd_dat_nobin_neg_pearson$result$act_set

simulated_curves_pearson_summary <- simulated_curves_pearson %>%
  inner_join(simulated_curves_pearson_act_set) %>%
  ...
  inner_join(simulated_curve_bmds_pearson) %>%
...
p_pearson <- ggplot(NULL, aes(conc, resp)) +
  ...
  geom_line(data = confident_curves_pearson[[1]], mapping = aes(x = conc, y = resp, group = sample_id), color = "green") +
  geom_line(data = confident_curves_pearson[[2]], mapping = aes(x = conc, y = resp, group = sample_id), color = "yellow") +
  geom_line(data = confident_curves_pearson[[3]], mapping = aes(x = conc, y = resp, group = sample_id), color = "blue") +
  ...
  geom_vline(data = bmds_pearson, aes(xintercept = POD_med), color = "yellow") +
  geom_vline(data = bmds_pearson, aes(xintercept = mean_POD), color = "green") +
  # geom_vline(data = bmds_pearson, aes(xintercept = POD_cil), color = "purple") +
...
```{r 88PlotAucEndpoints, include= FALSE}
auc_norm %>%
