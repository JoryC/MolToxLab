...
input_spearman_tib <- bmr_output_spearman_out %>%
  nest_join(simi_norm_tib_4rcurvep_spearman, by = c("endpoint"), keep = TRUE, name = "data") %>%
  select(RNGE, endpoint, bmr_exp, data)
```
...
sum_bmd_dat_nobin_neg_spearman <-
  summarize_rcurvep_output(
    bmd_dat_nobin_neg_spearman[[1]],
    inactivate = "INVERSE",
    ci_level = 0.95
  )
...
p_pearson <- ggplot(NULL, aes(conc, resp)) +
  geom_point(data = simulated_curves_pearson_summary, mapping = aes(x = conc, y  = resp, group = sample_id), color = "gray75", alpha = 0.2) +
  geom_point(data = curve_hits_pearson, mapping = aes(x = conc, y = resp, group = sample_id), color = "#a9a9a9",  alpha = 0.7) +
...
p_spearman <- ggplot(NULL, aes(conc, resp)) +
  geom_point(data = simulated_curves_spearman_summary, mapping = aes(x = conc, y = resp, group = sample_id), color = "gray75", alpha = 0.2) +
  geom_point(data = curve_hits_spearman, mapping = aes(x = conc, y = resp, group = sample_id), color = "#a9a9a9",  alpha = 0.7) +
...
```{r61.1.1Fittted_data, include=FALSE}
...
spearman_fitted_expr <- expression(value(future({
  run_fit(
    create_dataset(d = temp_spearman),
    modls = c("hill", "cnst"),
    keep_sets = c("fit_set", "resp_set"),
    n_samples = params$sample_size,
    hill_pdir = -1
...
ggsave(plot = p_pearson, filename = paste0("Output/Pearson_processed_curves_", params$sample_size, "_samples.svg"), device = "svg", units = "px", width = 1920, height = 1080, bg = "white", scale = 2)
ggsave(plot = p_spearman, filename = paste0("Output/Spearman_processed_curves_", params$sample_size, "_samples.svg"), device = "svg", units = "px", width = 1920, height = 1080, bg = "white", scale = 2)
ggsave(plot = p_spearman_fitted, filename = paste0("Output/Spearman_fitted_curves_", params$sample_size, "_samples.svg"), device = "svg", units = "px", width = 1920, height = 1080, bg = "white", scale = 2)
ggsave(plot = p_pearson_fitted, filename = paste0("Output/Pearson_fitted_curves_", params$sample_size, "_samples.svg"), device = "svg", units = "px", width = 1920, height = 1080, bg = "white", scale = 2)
```
