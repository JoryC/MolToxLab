...
#Pearson
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%#
# Rcurvep has a problem where if the hill model can't be fitted using a specific direction for any of the simulated responses, it just sets them all to NA. This is an issue because it makes plotting the failed models impossible. SO going to resample the responses for those chemicals that get flagged as NA so i can re-model them myself for images even though they go in the incorrect direction and/or too noisy.
#Index what chemicals are NAs only
test <- pearson_fitted$result$resp_set %>%
  group_by(chemical) %>%
  summarise(is_NaN = mean(resp, na.rm=TRUE))
  
NaNs <- as.vector(test[is.nan(test$is_NaN),][,1])

filter_1 <- pearson_fitted$result$resp_set %>%
  group_by(chemical) %>%
  filter(!chemical %in% NaNs[[1]])

filter_2 <- create_dataset(temp_pearson, n_samples = params$sample_size) %>%
  group_by(chemical) %>%
  filter(chemical %in% NaNs[[1]]) %>%
  mutate(direction = NA,
         fitted_resp = NA,
         sample_id = as.character(sample_id)) %>%
  mutate(resp = if_else(condition = resp < (-bmr_thresh_pearson), true = 0, false = resp))

replaced_NAs <- rbind(filter_1, filter_2)

pearson_fitted$result$resp_set <- replaced_NAs
...
pearson_fitted_output_summary <-
  summarise_fitted_results(
    ...
    fit_set = pearson_fitted_output$result$fit_set,
...
pearson_fitted_output_summary <- pearson_fitted_output_summary %>%
  group_by(chemical) %>%
...
fitted_hits_pearson <- fitted_hits_pearson %>%
  group_by(chemical) %>%
  mutate(x_label_num = if_else(condition = conc == lowest_conc, true = 0, false = (10^conc)*lowest_dose_treatment_group)) %>%
  mutate(x_label = factor(x_label_num, levels = c(0, lowest_dose_treatment_group, lowest_dose_treatment_group*10, lowest_dose_treatment_group*100, lowest_dose_treatment_group*1000, lowest_dose_treatment_group*10000, lowest_dose_treatment_group*100000, lowest_dose_treatment_group*1000000, lowest_dose_treatment_group*10000000, lowest_dose_treatment_group*100000000, lowest_dose_treatment_group*1000000000, lowest_dose_treatment_group*10000000000), ordered = TRUE))
...
saveRDS(pearson_fitted_output_summary, file = paste0("Data/", params$endpoint, "_fitted_output_summary_", params$sample_size, "_samples.RDS"))
saveRDS(bmds_fitted_pearson, file = paste0("Data/", params$endpoint, "_fitted_bmds_", params$sample_size, "_samples.RDS"))
```