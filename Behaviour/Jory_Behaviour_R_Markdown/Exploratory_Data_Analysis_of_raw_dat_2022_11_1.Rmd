...
```{r raw_behaviour_plots, echo=FALSE}
fishBehavDat$Dose <- factor(fishBehavDat$Dose, levels = c("Control", "Dose5", "Dose4", "Dose3", "Dose2", "Dose1"), ordered = TRUE)
p_raw_list <- list()
for (k in 1:length(chemicalNames)) {
  labels <-
    c(
      dplyr::select(
        .data = dplyr::filter(metaData, plate_id == chemicalNames[k]),
        Control
      )[[1]],
  ...
  p <- fishBehavDat %>%
    ...
    mutate(
      avgtotaldist = mean(totaldist, na.rm = TRUE),
      sdtotaldist = sd(totaldist, na.rm = TRUE)
    ) %>%
    ungroup() %>%
    ...
    #   aes(
    #     x = time_end,
    #     ymin = avgtotaldist + sdtotaldist,
    #     ymax = avgtotaldist - sdtotaldist,
    #     colour = Dose
    #   ),
    #   width = 1
    # ) +
    geom_point(
      aes(x = time_end, y = avgtotaldist, fill = Dose),
      shape = 21,
      colour = "black",
      size = 3
    ) +
    geom_line(aes(x = time_end, y = avgtotaldist, group = Dose)) +
    scale_fill_grey(
      start = 1,
      end = 0,
      labels = labels,
    ...
    p_raw_list[[chemicalNames[k]]] <- list(p, chemicalNames[k])
}
  
```

```{r raw_behaviour_plots_saving_images, echo=FALSE}
p1 <-
  gridExtra::grid.arrange(
    ...
    p_raw_list[[14]][[1]],
    p_raw_list[[15]][[1]],
    bottom = grid::textGrob("Time (min)",gp=grid::gpar(fontsize=18)),
    left = grid::textGrob("Total Distance moved (mm)", rot = 90, gp=grid::gpar(fontsize=18))
  )
ggsave(
  plot = p1,
  filename = "Part_1_Raw_Behavioural_Data.png",
...
ggsave(
  plot = p2,
  filename = "Part_2_Raw_Behavioural_Data.png",
  path = "~/MolToxLab/Behaviour/Jory_Behaviour_R_Markdown/Output",
  device = "png",
  scale = 4,
  units = "px",
...
# names(aov_list_all_chems_4_dunnett) <- chemicalNames
# names(pval_list_all_chems) <- chemicalNames
# aov_p_results <- plyr::ldply(pval_list_all_chems) %>%
#   rename(Cycle = .id, p_value = V1) %>%
#   dplyr::mutate(adj_p_value = p.adjust(p_value, method = "fdr")) %>%
#   dplyr::mutate(is_sig = if_else(condition = adj_p_value <= 0.05, true = TRUE, false= FALSE)) %>%
#   dplyr::mutate(adj_p_value = formatC(p_value, format = "e", digits = 2))
# 
# kable(aov_p_results, align = "lr", caption = paste("Table 2 - ANOVA test p-values after FDR adjustment, comparing effects of different genotypes on", params$value, "for each light cycle")) %>%
#     kable_styling(full_width = TRUE) %>%
#   scroll_box(height = "400px")
...
# We saw from preliminary data that fish take a bit of time to get acclimated to the machine and the responsiveness diminishes over time. So accounting for this observation in a two-way ANOVA model and totally ignoring any subgroup effects.
ANOVA_2 <- fishBehavDatBinLD %>%
  filter(Cycle %in% c("Dark", "Light")) %>%
  group_by(plate_id, Cycle) %>%
  mutate(Dose_mg_L = as.factor(Dose_mg_L),
...
ANCOVA_2 <- fishBehavDatBinLD %>%
  ...
  nest() %>%
  dplyr::mutate(model = map(data, ~ aov(
    totaldist ~ Dose_mg_L * time_end + Group,
    data = .
  ))) %>% #Where 'Group' is the dose group replicate... group A B or C for one of 3 petri dishes in the dose group... Tukey's results showed us that there is a significant interaction between the subgroups and the repsonse variable
  dplyr::select(model)

ANCOVACheck_2 <- ANCOVA_2 %>%
  mutate(model_tidy = map(model, broom::tidy)) %>%
  unnest(model_tidy)

ANCOVACheck_2 <- ANCOVACheck_2 %>%
  mutate(adj_p.value = p.adjust(p.value, method = "fdr")) %>%
  mutate(is.significant = if_else(
    condition = adj_p.value <= 0.05,
    true = TRUE,
...
DT::datatable(ANCOVACheck_2) #Final Data Frame for the ANOVA... will write later
...
DT::datatable(ANOVA_2_Sig_Results) #Just the significant results of the ANOVA
```

```{r Check_if_ANCOVA_and_one-way-ANOVA_models_are_different, echo=FALSE, include=FALSE}
...
names(model_check) <- chemicalNames_Cycle
...
write_csv(model_check_2, "Output/Comparison_one_way_ANOVA_and_ANCOVA.csv")
#These results tell me that adding the subgroup as a covariate significantly effects the results... Not in a good way>????
```
...
for (j in 1:length(chemicalNames_Cycle)) {
   temp_dun <- PMCMRplus::dunnettTest(ANOVA_2$model[[j]])
...
dunnett_binned <- ldply(dun_list, .id = "plate_id")
...
# for (j in 1:length(chemicalNames)) {
#   for (i in 1:2) {
#    temp_dun <- DunnettTest(aov_list_all_chems_4_dunnett[[j]][[i]])
# 
#     temp_dun_df <- as.data.frame(temp_dun$p.value[,1]) %>%
#       rownames_to_column( var = "Dose") %>%
#       rename(pval = "temp_dun$p.value[, 1]")
...
```{r Dunnett_TY3_New_code, echo=FALSE}
# #PostHoc tests
# set.seed(2345)
# 
...
# 
# fit_dunnett <- ANOVA %>%
#   dplyr::mutate(Dunnett = map(model, ~ glht(.x, linfct = mcp(Dose_mg_L="Dunnett")))) %>%
#   dplyr::mutate(tidy_Dunnett = map(Dunnett, ~ broom::tidy(.x))) %>%
#   dplyr::mutate(tidy_Dunnett_1 = map(tidy_Dunnett, ~ mutate(.x, plate_id = plate_id, Cycle = Cycle)))
# 
# Dunnett_comb <- plyr::ldply(fit_dunnett$tidy_Dunnett_1) %>%
#   dplyr::mutate(Dose_mg_L = str_split(contrast, pattern = " - ", simplify = TRUE)[,1]) %>%
#   dplyr::select(plate_id, Cycle, Dose_mg_L, estimate, std.error, statistic, adj.p.value) %>%
#   dplyr::mutate(pval = p.adjust(adj.p.value, method = "fdr")) %>% # p-values not already adjusted
#   dplyr::select(-adj.p.value)
# 
# Dunnett_comb_join_1 <- Dunnett_comb %>%
#   dplyr::filter(plate_id %in% c("TEG", "TGSH")) %>%
#   dplyr::filter(Cycle == "Light") %>%
#   dplyr::mutate_at(.vars = c("estimate", "std.error", "statistic", "pval"), function(x){x = NA})
# 
# Dunnett_comb_join_2 <- Dunnett_comb %>%
#   dplyr::filter(plate_id %in% c("24DMP")) %>%
#   dplyr::filter(Cycle == "Dark") %>%
#   dplyr::mutate_at(.vars = c("estimate", "std.error", "statistic", "pval"), function(x){x = NA})
# 
# Dunnett_comb_join_fixed_pvals <- rbind(Dunnett_comb_join_1, Dunnett_comb_join_2)
# 
# filtered_Dunnett_comb <- Dunnett_comb %>%
#   anti_join(Dunnett_comb_join_fixed_pvals, by = join_by(plate_id, Cycle, Dose_mg_L))
# 
# Dunnett_comb <- rbind(Dunnett_comb_join_fixed_pvals, filtered_Dunnett_comb)
```
...
ggsave(
  plot = p1,
  filename = "BehaviourTotaldistANOVA_Two-way_factorial_subgroup_DunnettResults_Part1.png",
  path = "~/MolToxLab/Behaviour/Jory_Behaviour_R_Markdown/Output",
  device = "png",
  scale = 4,
  units = "px",
...
ggsave(
  plot = p2,
  filename = "BehaviourTotaldistANOVA_Two-way_factorial_subgroup_DunnettResults_Part2.png",
  path = "~/MolToxLab/Behaviour/Jory_Behaviour_R_Markdown/Output",
  device = "png",
  scale = 4,
  units = "px",
...
AOV_datatable_binLD <- ANCOVACheck %>%
  dplyr::mutate(P_value = formatC(adj_p.value, format = "e", digits = 2)) %>%
  dplyr::select(plate_id, Cycle, term, df, sumsq, meansq, statistic, P_value)
```
...
write_csv(AOV_datatable_binLD, file = "Output/ANCOVA_Results_Raw_Data_Bin_Light_Dark.csv")
write_csv(dunnett_summary_all_chems_LD, file = "Output/DunnettTest_Results_Raw_Data_Bin_Light_Dark.csv")
for (i in seq_along(p_list)){
  temp <- p_list[[i]]
