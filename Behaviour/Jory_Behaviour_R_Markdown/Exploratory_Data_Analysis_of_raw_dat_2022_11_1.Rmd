...
input_behavdata <- fishBehavDat %>%
  ...
  mutate(totaldist = round(totaldist, digits = 3)) %>% #Bug with grubbs and large decimal places
...
behavdata_test <- fishBehavDat %>%
  mutate(totaldist = round(totaldist, digits = 3)) %>%
...
LeveneResults <- behavdata_ol_rm %>%
  group_by(plate_id) %>%
  summarise(car::leveneTest(totaldist, as.factor(Dose)))
...
var_check <- behavdata_ol_rm %>%
  group_by(plate_id, Dose) %>%
  mutate(variance = var(totaldist, na.rm = TRUE)) %>%
  summarise(variance) %>%
  unique()
...
for (j in 1:length(chemicalNames)){
  for (i in 1:6) {
    ...
    temp_aov <- aov(get(params$value) ~ Dose + Group, data = temp)
...
for (j in 1:length(chemicalNames)) {
  for (i in 1:6) {
    temp_dun <- dunnettT3Test(aov_list_all_chems_4_dunnett[[j]][[i]], p.adjust.method = "fdr")
...
summary_fishBehavDatBin <- fishBehavDatBin5 %>%
  ...
  summarise(se = sd(totaldist, na.rm = TRUE)/sqrt(length(totaldist)), value = mean(totaldist, na.rm = TRUE), sd = sd(totaldist, na.rm = TRUE))
```
...
for (j in 1:length(chemicalNames)) {
  for (i in 1:length(cycle_filter)) {
    temp <- aov_list_all_chems_4_dunnett[[j]][[i]] %>%
      broom::tidy() %>%
      mutate(plate_id = chemicalNames[j],
...
for (k in 1:length(chemicalNames)) {
  p <- dunnett_summary_all_chems %>%
    ...
    geom_tile(data = data.frame(Cycle = factor(c("Light_1","Dark_1","Light_2","Dark_2","Light_3","Dark_3"), levels = c("Light_1","Dark_1","Light_2","Dark_2","Light_3","Dark_3"), ordered = TRUE), alpha = c(0,1,0,1,0,1)), aes(x = Cycle, y = 0, height = Inf, width = 0.95, alpha = alpha),fill = "black") +
    scale_alpha_continuous(range = c(0, 0.2), guide = "none") +
    geom_col(aes(x = factor(Cycle, levels = c("Light_1","Dark_1","Light_2","Dark_2","Light_3","Dark_3"), ordered = TRUE)), position = "dodge", colour = "black") +
    geom_errorbar(aes(ymin = value + sd, ymax = value + sd),
                  width = .5,
                  position = position_dodge(0.9)) +
    geom_linerange(aes(ymin = value, ymax = value + sd, group = Dose), position = position_dodge(width = 0.9)) +
...
AOV_summary_all_chems_Bin5 <- AOV_summary_all_chems_Bin5 %>%
  mutate(P_value = p.adjust(p.value, method = "fdr")) %>%
  mutate(P_value = formatC(P_value, format = "e", digits = 2)) %>%
  select(Chemical, Cycle, term, df, sumsq, meansq, statistic, P_value)
```
...
LOEC_Summary <- dunnett_summary_all_chems %>%
  mutate(effect = if_else(condition = pval <= 0.05, true = TRUE, false = FALSE)) %>%
  filter(effect == TRUE) %>%
  mutate(Cycle2 = str_split(Cycle, pattern = "_", simplify = TRUE)[,1])
