---
title: "Similarity Endpoint: Spearman"
author: "Jory Curry"
...
```{r 46.2Visual_of_how_to_calculate_simi_enpoints, echo = FALSE}
 cor_dat <- fishBehavDat %>%
  ...
  summarise(cor = cor(x = as.matrix(.), method = "spearman", use = "pairwise.complete.obs"))
...
p1 <- fishBehavDat %>%
  ...
  geom_text(label = paste("Spearman's rank-order correlation = ", as.character(cor_dat$Control_1)), mapping = aes(x = 10, y = 100), inherit.aes = FALSE)
...
p2 <- fishBehavDat %>%
  ...
  geom_text(label = paste("Spearman's rank-order correlation = ", as.character(cor_dat$Control_9)), mapping = aes(x = 10, y = 100), inherit.aes = FALSE)
...
cor_dat_long %>%
  ...
  xlab("embryo_id") +
  ylab("Spearman's rank-order correlation")
```
...
```{r 61.1simulated_curveps, echo = FALSE}
simulated_curves <- sum_bmd_dat_nobin_neg$result$resp_set
simulated_curve_bmds <- sum_bmd_dat_nobin_neg$act_summary %>%
  group_by(chemical) %>%
  summarise(POD_med = POD_med, hit_confidence = hit_confidence, lowest_conc = lowest_conc)

simulated_curves %>%
  group_by(chemical, sample_id, conc) %>%
  ggplot(aes(x = conc, y = resp, group = sample_id), color = "gray75") +
  geom_line(alpha = 0.3) +
  geom_hline(yintercept = -bmr_thresh, linetype = "dashed", color = "red") +
  geom_vline(data = simulated_curve_bmds, aes(xintercept = POD_med), color = "blue") +
  geom_text(data = simulated_curve_bmds, mapping = aes(label = paste("Hit =", hit_confidence), x = lowest_conc+0.75), y = -85, inherit.aes = FALSE, size = 3) +
