...
chems_to_plot <- c(unique(dat$chemical))
dat <- dat %>%
...
#Or be lazy and import the data without wrangling
...
#Just Apical data
myTabledat <- dat %>%
  filter(Assay %in% c("Lethality & Deformity", "Swim Behaviour", "Metabolic")) %>%
  select(.id, chemical, min, max, median_POD, low_conf, high_conf, Assay, confidence)
