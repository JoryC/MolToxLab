...
for(i in 1:length(apicaldatasummary)) {
  ...
  ggplot(aes(x=Dose, y=Survival_Rate)) +
    ...
    ylab("Survival Rate")
    print(p)
    ggsave(names(apicaldatasummary[i]), device = "pdf", path = paste0(getwd(), "/Images/Survival_Rate"))
}
...
for(i in 1:length(apicaldatasummary)) {
  ...
  print(p)
  ggsave(names(apicaldatasummary[i]), device = "pdf", path = paste0(getwd(), "/Images/Deformity_Rate"))
}
