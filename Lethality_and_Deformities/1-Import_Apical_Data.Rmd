...
for(i in 1:length(apicaldata)) {
  temp2 <- aov(data = apicaldata[[i]], Survival.Rate ~ Dose..mg.L. + as.factor(Replicate))
...
for(i in 1:length(apicaldata)) {
  temp2 <- aov(data = apicaldata[[i]], Deformity.Rate ~ Dose..mg.L. + as.factor(Replicate))
...
for(i in 1:length(apicaldata)) {
  temp2 <- aov(data = apicaldata[[i]], Hatch.Rate ~ Dose..mg.L. + as.factor(Replicate))
...
for(i in 1:length(apicaldata)) {
  temp2 <- aov(data = apicaldata[[i]], Affected.Rate ~ Dose..mg.L. + as.factor(Replicate))
...
gg_data <- apicaldata_ol_rm %>%
  ...
  mutate(
    Affected.Rate_ol_rm = if_else(Affected.Rate_show == 1, true = Affected.Rate, false = NULL)
  ) %>%
...
apicaldatasummary_2 <- inner_join(apicaldatasummary_tibble, Dunnett_comb, by = c("Dose_mg_L", ".id")) %>%
  group_by(.id) %>%
  split(f = .$.id)
...
gg_ANOVA <- ancova_AR_summary %>%
  na.omit() %>%
  dplyr::filter(term %in% "Dose..mg.L.") %>%
  dplyr::rename(Chemical = .id) %>%
  dplyr::mutate(is.significant = if_else(condition = p.value < 0.05, true = TRUE, false = FALSE)) %>%
  dplyr::select(Chemical, term, p.value, is.significant)
