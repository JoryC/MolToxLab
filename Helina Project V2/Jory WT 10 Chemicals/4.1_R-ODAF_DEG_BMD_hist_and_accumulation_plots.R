...
#### load tPod Values ####
tPoD_values <- read.table(file = "BMDExpressData_R-ODAF/Output/tpod_values_R-ODAF_DEGs.txt", header = T)


#### BMD HISTOGRAM #####

# load BMD values for histograms 
raw_hist_data <- raw_importer(file_path = "BMDExpressData_R-ODAF/BMD/R-ODAF_DEGS/", 
                              file_type = ".txt", 
...
dataHist <- list()
for (i in chemnames) {
  if (any(is.na(histBreaks[[i]]))) {
    dataHist[[i]] <- data.frame(x = log10(highestdoses %>% filter(chemical == i) %>% pull(dose)), y = NA)
  } else {
    dataHist[[i]] <- hist(filtered_hist_data[[i]]$logBMD,
                          breaks = histBreaks[[i]],
                          plot = FALSE)
    dataHist[[i]] <-
      data.frame(x = dataHist[[i]]$mids, y = dataHist[[i]]$count)
  }
...
for(i in chemnames){
  if(!is.na(dataModes[[i]]$modes[1])){
    ...
    dataDens[[i]]<- data.frame(x = log10(highestdoses %>% filter(chemical == i) %>% pull(dose)), y = NA)
  }
}
...
for(i in chemnames){
  histPlots[[i]]<-ggplot(filtered_hist_data[[i]], aes(x = logBMD)) +
    ...
    # plot data
    {if(!is.null(dataModes[[i]]$bw))geom_histogram(binwidth = dataModes[[i]]$bw, color="black", fill=NA)} +
    {if(suppressWarnings(!any(is.na(dataDens[[i]]))))geom_line(data=dataDens[[i]], aes(x=x, y=y), inherit.aes = FALSE, size=1.5, col=hsv(0.5,1,0.8,0.4))} +
    # lines
...
multiplot(plotlist = histPlots, layout = matrix(c(1:length(histPlots), NA, NA), nrow=3, ncol = 3, byrow=TRUE))



#### REACTOME ACCUMULATION PLOT ####
# import data
raw_reactome_data <- raw_importer(file_path = "BMDExpressData_R-ODAF/REACTOME/R-ODAF_DEGs/", 
                                                file_type = ".txt", 
...
for(i in chemnames){
  reactomePlots[[i]] <- ggplot(filtered_reactome_data[[i]], aes(x = logBMD, y=rank)) + # can plot by "rank" or "count"
    ...
    geom_vline(xintercept = min(filtered_reactome_data[[i]]$logBMD), linetype = "dashed", color = "red") +
    # labels
...
multiplot(plotlist=reactomePlots, layout = matrix(c(1:length(reactomePlots), NA, NA), nrow=3, ncol=3, byrow=T))
...
raw_go_data <- raw_importer(file_path = "BMDExpressData_R-ODAF/GO_TERM/R_ODAF_DEGs/", 
                            file_type = ".txt", 
                            start_phrase = "GO/Pathway/Gene Set/Gene ID") %>%
...
for(i in chemnames){
  goPlots[[i]] <- ggplot(filtered_go_data[[i]], aes(x = logBMD, y=rank)) + 
    ...
    geom_vline(xintercept = min(filtered_go_data[[i]]$logBMD), linetype = "dashed", color = "red") +
    # labels
    labs(title = paste0(i, " (n=", nrow(filtered_go_data[[i]]), " GO terms)"), x = "logBMD", y = "Cumulative GO terms") +
    #theme
    theme_classic() +
...
multiplot(plotlist=goPlots, layout = matrix(c(1:length(goPlots), NA, NA), nrow=3, ncol=3, byrow=T))

write_csv(plyr::ldply(filtered_go_data), file = "All_Chemicals_GO_Data_R-ODAF_DEGs.csv")
write_csv(plyr::ldply(filtered_reactome_data), file = "All_Chemicals_Reactome_Data_R-ODAF_DEGs.csv")
write_csv(plyr::ldply(filtered_hist_data), file = "All_Chemicals_BMD_Data_for_Histogram_R-ODAF_DEGs.csv")  
