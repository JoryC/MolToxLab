...
#Import
go_raw_data <- raw_importer(file_path = "BMDExpressData_R-ODAF/GO_TERM/R_ODAF_DEGs/", 
                            file_type = ".txt", 
...
go_BMD_list <- list()
for (i in chemnames) {
  if (nrow(go_raw_data_filtered[[i]] > 0)) {
...
#Import
reactome_raw_data <- raw_importer(file_path = "BMDExpressData_R-ODAF/REACTOME/R-ODAF_DEGs/", 
                                     file_type = ".txt", 
                                     start_phrase = "GO/Pathway/Gene Set/Gene ID") %>%
...
for (i in chemnames) {
  if (nrow(reactome_raw_data_filtered[[i]] > 0)) {
    reactome_BMD_list[[i]] <-
...
BMD_raw_data <- list()
BMD_filenames <- list.files("BMDExpressData_R-ODAF/BMD/R-ODAF_DEGS", pattern = ".txt")
...
BMD_raw_data <- raw_importer(file_path = "BMDExpressData_R-ODAF/BMD/R-ODAF_DEGS/", 
                             file_type = ".txt", 
...
####Export Data#### 
saveRDS(go_BMD_list_logtransformed, "go_BMD_list_logtransformed_R-ODAF_DEGs.RDS")
saveRDS(reactome_BMD_list_logtransformed, "reactome_BMD_list_logtransformed_R-ODAF_DEGs.RDS")
saveRDS(all_BMD_list_logtransformed, "all_BMD_list_logtransformed_R-ODAF_DEGs.RDS")
