...
filenames <- list.files("BMDExpressData_R-ODAF/BMD/R-ODAF_DEGS", pattern = ".txt") %>% setNames(chemnames)
#check to make sure filenames match chemnames

# first import "sloppy data' to identify what row starts with "Probe Id"
row_start <- vector()
for (i in chemnames) {
  row_start[i] <-readLines(paste0("BMDExpressData_R-ODAF/BMD/R-ODAF_DEGS/", filenames[i])) %>%
  grepl("Probe Id.*", .) %>%
    which()
}
...
for (i in chemnames) {
  raw_data[[i]] <- suppressWarnings(suppressMessages(  # suppresses annoying parsing error warnings/messages
    read_delim(paste0("BMDExpressData_R-ODAF/BMD/R-ODAF_DEGS/", filenames[i]),
               skip=row_start[i]-1,
               delim="\t",
...
# export tPoDs
write.table(tpod_values, file = "BMDExpressData_R-ODAF/Output/tpod_values_R-ODAF_DEGs.txt", 
            quote = FALSE, 
            row.names = F,
            sep = "\t")
